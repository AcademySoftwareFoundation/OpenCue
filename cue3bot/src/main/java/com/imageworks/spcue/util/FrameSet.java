...
public class FrameSet {
    ...
    private ImmutableList<Integer> parseFrameRange(String frameRange) {
        ...
        for (String frameRangeSection : frameRange.split(",")) {
            builder.addAll(parseFrameRangeSection(frameRangeSection));
        }
        return builder.build();
    }

    private ImmutableList<Integer> parseFrameRangeSection(String frameRange) {
        ...
        throw new IllegalArgumentException("unrecognized frame range section " + frameRange);
    }

    private ImmutableList<Integer> getIntRange(Integer start, Integer end, Integer step) {
    ...
    private ImmutableList<Integer> getSteppedRange(
    ...
    private ImmutableList<Integer> getInterleavedRange(Integer start, Integer end, Integer step) {
        validateStepSign(start, end, step);
    ...
    private void validateStepSign(Integer start, Integer end, Integer step) {
        if (step > 1) {
            if (end < start) {
