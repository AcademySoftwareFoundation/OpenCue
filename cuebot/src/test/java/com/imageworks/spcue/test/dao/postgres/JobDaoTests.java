...
public class JobDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void testUpdateJobState() {
    ...
    assertEquals(JobState.FINISHED.toString(), jdbcTemplate
  ...
  public void testIsOverMaxCores() {
    ...
    jdbcTemplate.update("UPDATE job_resource SET int_cores = 450 WHERE pk_job=?", job.getJobId());
  ...
  public void testMaxCoreTrigger() {
    ...
    int maxCores = jdbcTemplate.queryForObject(
        "SELECT int_max_cores FROM job_resource WHERE pk_job=?", Integer.class, job.getJobId());
  ...
  public void testUpdateJobPriority() {
    ...
    assertEquals(Integer.valueOf(199), jdbcTemplate.queryForObject(
        "SELECT int_priority FROM job_resource WHERE pk_job=?", Integer.class, job.getJobId()));
  }
  ...
  public void testUpdateJobMinCores() {
    ...
    assertEquals(Integer.valueOf(100), jdbcTemplate.queryForObject(
        "SELECT int_min_cores FROM job_resource WHERE pk_job=?", Integer.class, job.getJobId()));
  }
  ...
  public void testUpdateJobPaused() {
    ...
    assertTrue(jdbcTemplate.queryForObject("SELECT b_paused FROM job WHERE pk_job=?", Boolean.class,
        job.getJobId()));
  ...
  public void testUpdateJobLogPath() {
    ...
    assertEquals(newLogDir, jdbcTemplate
        .queryForObject("SELECT str_log_dir FROM job WHERE pk_job=?", String.class, job.id));
  }
  ...
  public void mapPostJob() {
    JobSpec spec =
        jobLauncher.parse(new File("src/test/resources/conf/jobspec/jobspec_postframes.xml"));
    jobLauncher.launch(spec);

    final String pk_job = spec.getJobs().get(0).detail.id;

    assertEquals(Integer.valueOf(1), jdbcTemplate
        .queryForObject("SELECT COUNT(*) FROM job_post WHERE pk_job=?", Integer.class, pk_job));
  }
  ...
  public void activatePostJob() {
    ...
    assertEquals(JobState.PENDING.toString(),
        jdbcTemplate.queryForObject("SELECT str_state FROM job WHERE pk_job=?", String.class,
  ...
  public void testUpdateUsage() {
    ...
    assertEquals(Long.valueOf(usage.getClockTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_clock_time_success FROM job_usage WHERE pk_job=?", Long.class, job.getId()));

    assertEquals(Long.valueOf(usage.getCoreTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_core_time_success FROM job_usage WHERE pk_job=?", Long.class, job.getId()));

    assertEquals(Integer.valueOf(1),
        jdbcTemplate.queryForObject("SELECT int_frame_success_count FROM job_usage WHERE pk_job=?",
    ...
    assertEquals(Long.valueOf(usage.getClockTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_clock_time_fail FROM job_usage WHERE pk_job=?", Long.class, job.getId()));

    assertEquals(Long.valueOf(usage.getCoreTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_core_time_fail FROM job_usage WHERE pk_job=?", Long.class, job.getId()));

    assertEquals(Integer.valueOf(1), jdbcTemplate.queryForObject(
        "SELECT int_frame_fail_count FROM job_usage WHERE pk_job=?", Integer.class, job.getId()));
  }
