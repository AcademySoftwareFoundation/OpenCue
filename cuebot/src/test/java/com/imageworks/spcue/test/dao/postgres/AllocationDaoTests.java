...
public class AllocationDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void testDeleteAllocationWithProc() {
    ...
    assertEquals(Integer.valueOf(1),
        jdbcTemplate.queryForObject(
            "SELECT COUNT(1) FROM alloc WHERE pk_alloc=? AND b_enabled = false", Integer.class,
            alloc.getAllocationId()));

    assertEquals(ALLOC_FQN,
        jdbcTemplate.queryForObject(
            "SELECT str_name FROM alloc WHERE pk_alloc=? AND b_enabled = false", String.class,
            alloc.getAllocationId()));
    ...
    assertEquals(Integer.valueOf(1),
        jdbcTemplate.queryForObject(
            "SELECT COUNT(1) FROM alloc WHERE pk_alloc=? AND b_enabled = true", Integer.class,
            alloc.getAllocationId()));
  }
  ...
  public void testUpdateAllocationTag() {
    ...
    assertEquals("foo", jdbcTemplate.queryForObject("SELECT str_tag FROM alloc WHERE pk_alloc=?",
