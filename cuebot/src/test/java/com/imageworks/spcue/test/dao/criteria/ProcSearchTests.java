...
public class ProcSearchTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void testFilterByHost() {
    ...
    assertEquals(1, foundProcs.size());
    assertThat(foundProcs.stream().map(proc -> hostManager.getVirtualProc(proc.getId()).hostName)
  ...
  private void launchJobs() {
    ...
    File file =
        new File(classLoader.getResource("conf/jobspec/jobspec_dispatch_test.xml").getFile());
  ...
  private RenderHost.Builder buildRenderHost() {
    return RenderHost.newBuilder().setBootTime(1192369572)
        ...
        .setNumProcs(1).setCoresPerProc(100).addTags("test").setState(HardwareState.UP)
        .setFacility("spi").putAttributes("SP_OS", "Linux");
  }
