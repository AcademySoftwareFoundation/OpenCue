...
public class HostReportHandlerTests extends TransactionalTest {
  ...
  private static CoreDetail getCoreDetail(int total, int idle, int booked, int locked) {
    return CoreDetail.newBuilder().setTotalCores(total).setIdleCores(idle).setBookedCores(booked)
        .setLockedCores(locked).build();
  }
  ...
  private static RenderHost.Builder getRenderHostBuilder(String hostname) {
    return RenderHost.newBuilder().setName(hostname).setBootTime(1192369572)
        ...
        .setState(HardwareState.UP).setFacility("spi").putAttributes("SP_OS", "Linux").setNumGpus(0)
        .setFreeGpuMem(0).setTotalGpuMem(0);
  }
  ...
  public void testHandleHostReport() throws InterruptedException {
    ...
    HostReport report2 =
        HostReport.newBuilder().setHost(getRenderHost(hostname2)).setCoreInfo(cores).build();
  ...
  public void testHandleHostReportWithNewAllocation() {
    FacilityInterface facility = adminManager.getFacility("AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAA0");
    ...
    CoreDetail cores = getCoreDetail(200, 200, 0, 0);
    HostReport report =
        HostReport.newBuilder().setHost(getNewRenderHost("test")).setCoreInfo(cores).build();
  ...
  public void testHandleHostReportWithExistentAllocation() {
    ...
    CoreDetail cores = getCoreDetail(200, 200, 0, 0);
    HostReport report =
        HostReport.newBuilder().setHost(getNewRenderHost("general")).setCoreInfo(cores).build();
  ...
  public void testHandleHostReportWithNonExistentTags() {
    ...
    CoreDetail cores = getCoreDetail(200, 200, 0, 0);
    HostReport report =
        HostReport.newBuilder().setHost(getNewRenderHost("nonexistent")).setCoreInfo(cores).build();
  ...
  public void testHandleHostReportWithFullTemporaryDirectories() {
     ...
     * freeTempDir than the threshold (opencue.properties: min_available_temp_storage_percentage)
     * Postcondition: - Host hardwareState changes to REPAIR - A comment is created with
     * subject=SUBJECT_COMMENT_FULL_TEMP_DIR and user=CUEBOT_COMMENT_USER
     */
    ...
    HostReport report2 = HostReport.newBuilder()
        .setHost(getRenderHostBuilder(hostname).setFreeMcp(CueUtil.GB).build()).setCoreInfo(cores)
        .build();
  ...
  public void testHandleHostReportWithHardwareStateRepairNotRelatedToFullTempDir() {
     ...
     *
     * - There is no comment with subject=SUBJECT_COMMENT_FULL_MCP_DIR and user=CUEBOT_COMMENT_USER
     * associated with the host The host.hardwareState continue as HardwareState.REPAIR
     */
    // Create HostReport
  ...
  public void testMemoryAndLlu() {
    ...
    RunningFrameInfo info = RunningFrameInfo.newBuilder().setJobId(proc.getJobId())
        .setLayerId(proc.getLayerId()).setFrameId(proc.getFrameId()).setResourceId(proc.getProcId())
        .setLluTime(now / 1000).setMaxRss(420000).build();
    HostReport report = HostReport.newBuilder().setHost(getRenderHost(hostname)).setCoreInfo(cores)
        .addFrames(info).build();
  ...
  public void testMemoryAggressionRss() {
    ...
    RunningFrameInfo info = RunningFrameInfo.newBuilder().setJobId(proc.getJobId())
        .setLayerId(proc.getLayerId()).setFrameId(proc.getFrameId()).setResourceId(proc.getProcId())
        .setRss(memoryOverboard).setMaxRss(memoryOverboard).build();
  ...
  public void testMemoryAggressionMaxRss() {
    ...
    RunningFrameInfo info = RunningFrameInfo.newBuilder().setJobId(proc.getJobId())
        .setLayerId(proc.getLayerId()).setFrameId(proc.getFrameId()).setResourceId(proc.getProcId())
        .setRss((long) Math.ceil(0.95 * proc.memoryReserved)).setMaxRss(memoryOverboard).build();
    HostReport report = HostReport.newBuilder().setHost(getRenderHost(hostname))
        .setCoreInfo(getCoreDetail(200, 200, 0, 0)).addFrames(info).build();
  ...
  public void testMemoryAggressionMemoryWarning() {
    ...
    RunningFrameInfo info2 = RunningFrameInfo.newBuilder().setJobId(proc2.getJobId())
        ...
        .setResourceId(proc2.getProcId()).setUsedSwapMemory(CueUtil.MB512).setVsize(CueUtil.GB4)
        .setRss(CueUtil.GB4).setMaxRss(CueUtil.GB4).build();
    ...
    RunningFrameInfo info3 = RunningFrameInfo.newBuilder().setJobId(proc3.getJobId())
        ...
        .setVsize(memoryUsedProc3).setRss(memoryUsedProc3).setMaxRss(memoryUsedProc3).build();
    ...
    HostReport report =
        HostReport.newBuilder().setHost(hostAfterUpdate).setCoreInfo(getCoreDetail(200, 200, 0, 0))
    ...
    FrameCompleteReport completeReport = FrameCompleteReport.newBuilder().setHost(hostAfterUpdate)
        .setFrame(runningFrame).setExitSignal(9).setRunTime(1).setExitStatus(1).build();
