...
public class FrameDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void create() {
    ...
    RenderHost host = RenderHost.newBuilder().setName(HOST).setBootTime(1192369572)
        ...
        .setNumProcs(1).setCoresPerProc(100).addAllTags(ImmutableList.of("mcore", "4core", "8g"))
  ...
  @AfterTransaction
  public void destroy() {
    jdbcTemplate.update("DELETE FROM host WHERE str_name=?", HOST);
  }
  ...
  public void testFindFrames() {
    ...
    r.setCriteria(criteria.toBuilder().addFrames("0001-pass_1").build());
  ...
  public void testgetOrphanedFrames() {
    ...
    /*
     * Update the first frame to the orphan state, which is a frame that is in the running state,
     * has no corresponding proc entry and has not been updated in the last 5 min.
     */
    jdbcTemplate.update(
  ...
  public void testMarkFrameAsWaiting() {
    ...
    assertEquals(Integer.valueOf(1), jdbcTemplate.queryForObject(
        "SELECT int_depend_count FROM frame WHERE pk_frame=?", Integer.class, f.getFrameId()));
  ...
  public void testMarkFrameAsDepend() {
    ...
    assertTrue(jdbcTemplate.queryForObject("SELECT b_active FROM depend WHERE pk_layer_depend_er=?",
    ...
    /*
     * Need to grab new version of frame object once the state has changed.
     */
  ...
  public void testGetDependentFrames() {
    ...
    dependManager.createDepend(new FrameOnFrame(frame_a, frame_b));

    assertEquals(1,
        frameDao.getDependentFrames(dependManager.getWhatDependsOn(frame_b).get(0)).size(), 1);
  }
  ...
  public void testSetCheckpointState() {
    ...
    String state =
        jdbcTemplate.queryForObject("SELECT str_checkpoint_state FROM frame WHERE pk_frame=?",
    ...
    Map<String, Object> result = jdbcTemplate
        .queryForMap("SELECT int_checkpoint_count FROM frame WHERE pk_frame=?", frame.getFrameId());
  ...
  public void testUpdateFrameOverride() {
    ...
    FrameStateDisplayOverride overrideUpdate =
        createFrameStateDisplayOverride(FrameState.SUCCEEDED, "DONE", 100, 100, 100);
