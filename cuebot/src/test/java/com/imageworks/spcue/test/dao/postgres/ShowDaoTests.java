...
public class ShowDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void testInsertShow() {
    ...
    assertEquals(Integer.valueOf(1), jdbcTemplate
        .queryForObject("SELECT count(*) FROM show where str_name=?", Integer.class, show.name));
  ...
  public void testUpdateShowDefaultMinCores() {
    ...
    assertTrue(jdbcTemplate.queryForObject("SELECT int_default_min_cores FROM show WHERE pk_show=?",
  ...
  public void testUpdateShowDefaultMaxCores() {
    ...
    assertTrue(jdbcTemplate.queryForObject("SELECT int_default_max_cores FROM show WHERE pk_show=?",
  ...
  public void testUpdateShowCommentEmail() {
    ...
    String email = jdbcTemplate.queryForObject("SELECT str_comment_email FROM show WHERE pk_show=?",
  ...
  public void testUpdateBookingEnabled() {
    ...
    assertFalse(jdbcTemplate.queryForObject("SELECT b_booking_enabled FROM show WHERE pk_show=?",
  ...
  public void testUpdateActive() {
    ...
    assertFalse(jdbcTemplate.queryForObject("SELECT b_active FROM show WHERE pk_show=?",
        Boolean.class, show.id));
    showDao.updateActive(show, true);
    assertTrue(jdbcTemplate.queryForObject("SELECT b_active FROM show WHERE pk_show=?",
        Boolean.class, show.id));
  }
  ...
  public void testUpdateFrameCounters() {
    ...
    int frameSuccess = jdbcTemplate.queryForObject(
        "SELECT int_frame_success_count FROM show_stats WHERE pk_show=?", Integer.class, show.id);
    ...
    int frameFail = jdbcTemplate.queryForObject(
        "SELECT int_frame_fail_count FROM show_stats WHERE pk_show=?", Integer.class, show.id);
