...
public class HostDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public static RenderHost buildRenderHost(String name) {
    RenderHost host = RenderHost.newBuilder().setName(name).setBootTime(1192369572)
        ...
        .setFreeMcp(CueUtil.GB).setFreeMem(15290520).setFreeSwap((int) CueUtil.MB512).setLoad(1)
        .setNimbyEnabled(false).setTotalMcp(CueUtil.GB4).setTotalMem((int) CueUtil.GB16)
        .setTotalSwap((int) CueUtil.GB2).setNimbyEnabled(false).setNumProcs(2).setCoresPerProc(400)
  ...
  public void testInsertHost() {
    hostDao.insertRenderHost(buildRenderHost(TEST_HOST), hostManager.getDefaultAllocationDetail(),
        false);

    assertEquals(Long.valueOf(CueUtil.GB16 - this.MEM_RESERVED_SYSTEM), jdbcTemplate
        .queryForObject("SELECT int_mem FROM host WHERE str_name=?", Long.class, TEST_HOST));
  }
  ...
  public void testInsertHostFQDN1() {
    ...
    hostDao.insertRenderHost(buildRenderHost(FQDN_HOST), hostManager.getDefaultAllocationDetail(),
        true);
  ...
  public void testInsertHostAlternateOS() {
    ...
    assertEquals("spinux1",
        jdbcTemplate.queryForObject("SELECT str_os FROM host_stat, host "
            + "WHERE host.pk_host = host_stat.pk_host " + "AND host.str_name=?", String.class,
            TEST_HOST),
        "spinux1");
  }
  ...
  public void testUpdateThreadMode() {
    ...
    assertEquals(Integer.valueOf(ThreadMode.AUTO_VALUE), jdbcTemplate
        .queryForObject("SELECT int_thread_mode FROM host WHERE pk_host=?", Integer.class, d.id));

    hostDao.updateThreadMode(d, ThreadMode.ALL);

    assertEquals(Integer.valueOf(ThreadMode.ALL_VALUE), jdbcTemplate
        .queryForObject("SELECT int_thread_mode FROM host WHERE pk_host=?", Integer.class, d.id));
  }
  ...
  public void testDeleteDownHosts() {
    for (int i = 0; i < 3; i++) {
      ...
      hostDao.insertRenderHost(buildRenderHost(name), hostManager.getDefaultAllocationDetail(),
          false);
      if (i != 1) {
  ...
  public void updateHostStats() {
    ...
    Map<String, Object> result = jdbcTemplate.queryForMap("SELECT * FROM host_stat WHERE pk_host=?",
  ...
  public void testGetStrandedCoreUnits() {
    ...
    // whole core properly.
    jdbcTemplate.update("UPDATE host SET int_cores_idle=150, int_mem_idle = ? WHERE pk_host = ?",
