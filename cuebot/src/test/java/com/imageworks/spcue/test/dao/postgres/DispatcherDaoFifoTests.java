...
public class DispatcherDaoFifoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  private void launchJobs(int count) throws Exception {
    Document docTemplate =
        new SAXBuilder(true).build(new File("src/test/resources/conf/jobspec/jobspec_simple.xml"));
    ...
    for (int i = 0; i < count; i++) {
      ...
      jdbcTemplate.update("UPDATE job SET ts_started = ? WHERE str_name = ?", new Timestamp(t + i),
          "pipe-default-testuser_job" + i);
    }
  }
  ...
  public void createHost() {
    RenderHost host = RenderHost.newBuilder().setName(HOSTNAME).setBootTime(1192369572)
        ...
        .setNumProcs(2).setCoresPerProc(100).addTags("test").setState(HardwareState.UP)
        .setFacility("spi").putAttributes("SP_OS", "Linux").build();
  ...
  public void testFifoSchedulingDisabled() throws Exception {
    ...
    Collections.sort(sortedJobs, Comparator.comparing(jobId -> jobManager.getJob(jobId).getName()));

    for (int i = 0; i < count; i++) {
      assertEquals("pipe-default-testuser_job" + i, jobManager.getJob(sortedJobs.get(i)).getName());
    }
  }
  ...
  public void testGroup() throws Exception {
    ...
    Set<String> jobs = dispatcherDao.findDispatchJobs(getHost(), groupManager.getGroupDetail(job));
    assertEquals(count, jobs.size());
  }
}
