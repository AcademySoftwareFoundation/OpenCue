...
public class ProcDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public DispatchHost createHost() {
    ...
    RenderHost host = RenderHost.newBuilder().setName("beta").setBootTime(1192369572)
        ...
        .setNimbyEnabled(false).setNumProcs(8).setCoresPerProc(100).setState(HardwareState.UP)
  ...
  public void testDontVerifyRunningProc() {
    ...
    assertEquals("RUNNING", jdbcTemplate
        .queryForObject("SELECT str_state FROM frame WHERE pk_frame=?", String.class, frame.id));
  ...
  public void testGetVirtualProc() {
    ...
    assertEquals(Integer.valueOf(1), jdbcTemplate
        .queryForObject("SELECT COUNT(*) FROM host WHERE pk_host=?", Integer.class, host.id));
    ...
    assertEquals(Integer.valueOf(1), jdbcTemplate
        .queryForObject("SELECT COUNT(*) FROM proc WHERE pk_proc=?", Integer.class, proc.id));
  ...
  public void testGetReservedMemory() {
    ...
    assertEquals(Long.valueOf(this.MEM_RESERVED_DEFAULT), jdbcTemplate
        .queryForObject("SELECT int_mem_reserved FROM proc WHERE pk_proc=?", Long.class, _proc.id));
