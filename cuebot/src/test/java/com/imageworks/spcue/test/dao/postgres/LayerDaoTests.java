...
public class LayerDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public void updateMinGpuMemory() {
    ...
    assertEquals(Long.valueOf(mem), jdbcTemplate.queryForObject(
        "SELECT int_gpu_mem_min FROM layer WHERE pk_layer=?", Long.class, layer.getLayerId()));
  }
  ...
  public void updateMinCores() {
    ...
    assertEquals(Integer.valueOf(cores), jdbcTemplate.queryForObject(
        "SELECT int_cores_min FROM layer WHERE pk_layer=?", Integer.class, layer.getLayerId()));
  }
  ...
  public void updateMaxCores() {
    ...
    assertEquals(Integer.valueOf(cores), jdbcTemplate.queryForObject(
        "SELECT int_cores_max FROM layer WHERE pk_layer=?", Integer.class, layer.getLayerId()));
  }
  ...
  public void isOptimizable() {
    ...
    /*
     * Take the general tag away. If a layer is not a general layer it cannot be optmiized. Assert
     * False
     */
  ...
  public void testUpdateUsage() {
    ...
    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject(
            "SELECT int_clock_time_success FROM layer_usage WHERE pk_layer=?", Integer.class,
            layer.getId()));

    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject(
            "SELECT int_core_time_success FROM layer_usage WHERE pk_layer=?", Integer.class,
            layer.getId()));

    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject(
            "SELECT int_frame_success_count FROM layer_usage WHERE pk_layer=?", Integer.class,
            layer.getId()));

    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject("SELECT int_clock_time_fail FROM layer_usage WHERE pk_layer=?",
            Integer.class, layer.getId()));

    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject("SELECT int_core_time_fail FROM layer_usage WHERE pk_layer=?",
            Integer.class, layer.getId()));

    assertEquals(Integer.valueOf(0),
        jdbcTemplate.queryForObject("SELECT int_frame_fail_count FROM layer_usage WHERE pk_layer=?",
            Integer.class, layer.getId()));
    ...
    assertEquals(Long.valueOf(usage.getClockTimeSeconds()),
        jdbcTemplate.queryForObject(
            "SELECT int_clock_time_success FROM layer_usage WHERE pk_layer=?", Long.class,
            layer.getId()));

    assertEquals(Long.valueOf(usage.getCoreTimeSeconds()),
        jdbcTemplate.queryForObject(
            "SELECT int_core_time_success FROM layer_usage WHERE pk_layer=?", Long.class,
            layer.getId()));
    ...
    assertEquals(Long.valueOf(usage.getClockTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_clock_time_fail FROM layer_usage WHERE pk_layer=?", Long.class, layer.getId()));

    assertEquals(Long.valueOf(usage.getCoreTimeSeconds()), jdbcTemplate.queryForObject(
        "SELECT int_core_time_fail FROM layer_usage WHERE pk_layer=?", Long.class, layer.getId()));
  ...
  public void isLayerThreadable() {
    LayerDetail layer = getLayer();
    jdbcTemplate.update("UPDATE layer set b_threadable = false WHERE pk_layer = ?", layer.getId());

    assertFalse(layerDao.isThreadable(layer));

    jdbcTemplate.update("UPDATE layer set b_threadable = true WHERE pk_layer = ?", layer.getId());
  ...
  public void testAddLimit() {
    ...
    List<String> sourceIds = Arrays.asList(getTestLimitId(LIMIT_NAME), getTestLimitId(LIMIT_TEST_A),
        getTestLimitId(LIMIT_TEST_B), getTestLimitId(LIMIT_TEST_C));
    List<String> resultIds =
        Arrays.asList(limits.get(0).id, limits.get(1).id, limits.get(2).id, limits.get(3).id);
