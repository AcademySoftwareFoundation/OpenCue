...
public class JobManagerTests extends AbstractTransactionalJUnit4SpringContextTests {
  ...
  public DispatchHost createHost() {
    ...
    RenderHost host = RenderHost.newBuilder().setName("test_host").setBootTime(1192369572)
        ...
        .setFacility("spi").addTags("general").build();
  ...
  public void init() {
    ...
    JobSpec spec =
        jobLauncher.parse(new File("src/test/resources/conf/jobspec/jobspec_dispatch_test.xml"));
  ...
  public void testNonExistentShow() {
    JobSpec spec =
        jobLauncher.parse(new File("src/test/resources/conf/jobspec/jobspec_nonexistent_show.xml"));
    try {
      jobLauncher.launch(spec);
