...
public interface JobManager {
  ...
  /**
   * Removes an existing job entry. The job must be in the Finished state before it can be removed.
   *
   * @param JobDetail job
   */
  ...
  /**
   * Shutting down a job will signal RQD to kill all frames and drop all dependencies for specified
   * job. Job is put into Shutdown state which should be commited before any other operations are
   * done on the job. When shutdown is complete, the job shoud be marked Finished.
   *
   * @param JobDetail job
   */
  ...
  /**
   * Marks a specific frame as waiting, setting its dependency count to 0 in the process even though
   * it has active dependencies.
   *
   * @param frame
   */
   ...
   * undo a markFrameAsWaiting. If the frame has no active depends this call should have no effect.
   *
   * @param frame
   */
