...
public class MaintenanceManagerSupport {
   ...
   * comes back up, down proc checks will not resume for WAIT_FOR_HOST_REPORTS_MS milliseconds. This
   * is to give procs a chance to report back in.
   *
   */
    ...
    try {
      ...
      try {
        ...
        if (hosts > 0) {
          ...
          boolean autoDeleteDownHosts =
              env.getProperty("maintenance.auto_delete_down_hosts", Boolean.class, false);
  ...
  private void clearOrphanedProcs() {
    ...
    for (FrameInterface frame : frames) {
      try {
        frameDao.updateFrameStopped(frame, FrameState.WAITING, Dispatcher.EXIT_STATUS_FRAME_ORPHAN);
      } catch (Exception e) {
