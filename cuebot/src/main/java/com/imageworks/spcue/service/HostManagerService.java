...
public class HostManagerService implements HostManager {
  ...
  public void setHostStatistics(HostInterface host, long totalMemory, long freeMemory,
    ...
    hostDao.updateHostStats(host, totalMemory, freeMemory, totalSwap, freeSwap, totalMcp, freeMcp,
        totalGpuMemory, freeGpuMemory, load, bootTime, os);
  }
    ...
    if (rhost.getTagsCount() > 0) {
      ...
      for (String tag : rhost.getTagsList()) {
        try {
          ...
          logger.info("set " + rhost.getName() + " to the given allocation " + alloc.getName());
          break;
