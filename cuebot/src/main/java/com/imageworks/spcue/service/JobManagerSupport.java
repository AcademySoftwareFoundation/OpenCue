...
public class JobManagerSupport {
    ...
    if (isManualKill && source.getReason().isEmpty()) {
      ...
      if (jobManager.shutdownJob(job)) {
        ...
        if (isManualKill) {
          ...
          /**
           * Sleep a bit here in case any frames were dispatched during the job shutdown process.
           */
          try {
            Thread.sleep(3000);
          } catch (InterruptedException e1) {
            logger.info(job.getName() + "/" + job.getId() + " shutdown thread was interrupted.");
          ...
          FrameStateSeq states =
              newCriteria.getStates().toBuilder().addFrameStates(FrameState.RUNNING).build();
          ...
          for (FrameInterface frame : jobManager.findFrames(search)) {
            ...
            if (manualStopFrame(frame, FrameState.WAITING)) {
              try {
                ...
                logger.warn("Failed to kill frame " + frame + " on job shutdown operation, " + e);
              } catch (Exception e) {
        ...
        /*
         * Send mail after all frames have been stopped or else the email will have inaccurate
         * numbers.
         */
  ...
  public void satisfyWhatDependsOn(FrameInterface frame) {
    ...
    logger.info(
        "satisfying " + depends.size() + " depends that are waiting on frame " + frame.getName());
  ...
  public void satisfyWhatDependsOn(LayerInterface layer) {
    ...
    logger.info(
        "satisfying " + depends.size() + " depends that are waiting on layer " + layer.getName());
  ...
  public void satisfyWhatDependsOn(JobInterface job) {
    ...
    logger
        .info("satisfying " + depends.size() + " depends that are waiting on job " + job.getName());
  ...
  /*
   * Destructive functions require a extra Source argument which contains information about the user
   * making the call. This information is propagated down to the frame log file.
   *
  ...
  /**
   * Kill a list procs. If RQD throws back an exception, the proc is considered lost and is manually
   * removed.
   *
   * @param procs
   * @param source
  ...
  public void retryFrames(FrameSearchInterface request, Source source) {
    for (FrameInterface frame : jobManager.findFrames(request)) {
      try {
        ...
        CueExceptionUtil.logStackTrace("Failed to retry frame " + frame + " from source " + source,
            e);
      }
    }
  }
  ...
  /**
   * Eat frames that match the specified FrameSearch. Eaten frames are considered "Succeeded" by the
   * dispatcher. A Job with all eaten frames will leave the cue.
   *
   * @param request
   * @param source
   */
  ...
  /**
   * Eat the specified frame. Eaten frames are considered "Succeeded" by the dispatcher. A Job with
   * all eaten frames will leave the cue.
   *
   * @param frame
   * @param source
   */
  ...
  /**
   * Stops the specified frame. Return true if the call to this method actually stops the frame, ie
   * the state changes from Running to the given state. Return false if the frame was already
   * stopped.
   *
   * Stopping the frame also removes the link between the frame and the proc. The proc still exists,
   * but, its assigned frame is null.
   *
   * @param frame
   * @param state
   */
