    ...
    try {
      ...
      for (LayerDetail layer : layers) {
        if (layer.type.equals(LayerType.RENDER)) {
          ...
          stats.setOutputs(
              jobManager.getLayerOutputs(layer).stream().sorted().collect(Collectors.toList()));
      ...
      for (String email : d.email.split(",")) {
        try {
          CueUtil.sendmail(email, this.emailFromAddress, subject, new StringBuilder(w.toString()),
              imageMap, file);
        } catch (Exception e) {
