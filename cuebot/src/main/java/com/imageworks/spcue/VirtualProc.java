...
public class VirtualProc extends FrameEntity implements ProcInterface {
     ...
     *        the thread-mode idle-core expansion. The whole-farm Maestro uses this: it has already
     *        scored the placement and decremented its in-memory snapshot by the requested cores, so
     *        letting build() silently reserve more (grab-idle) would corrupt that accounting and
     *        make it over-plan a host whose cores a prior frame already swallowed. Maestro fills
     *        hosts by planning several placements, not by one frame ballooning. Explicit whole-host
