...
public class BookingDaoJdbc extends JdbcDaoSupport implements BookingDao {
  ...
  public void insertLocalHostAssignment(HostInterface h, JobInterface job, LocalHostAssignment l) {
    ...
    getJdbcTemplate().update(INSERT_LOCAL_JOB_ASSIGNMENT, l.id, job.getJobId(), l.getLayerId(),
        l.getFrameId(), l.getType().toString(), h.getHostId(), l.getMaxMemory(), l.getMaxMemory(),
        l.getMaxCoreUnits(), l.getMaxCoreUnits(), l.getMaxGpuMemory(), l.getMaxGpuMemory(),
        l.getMaxGpuUnits(), l.getMaxGpuUnits(), l.getThreads());
  }

  @Override
  public void insertLocalHostAssignment(HostInterface h, LayerInterface layer,
      LocalHostAssignment l) {
    l.id = SqlUtil.genKeyRandom();
    ...
    getJdbcTemplate().update(INSERT_LOCAL_JOB_ASSIGNMENT, l.id, l.getJobId(), l.getLayerId(),
        l.getFrameId(), l.getType().toString(), h.getHostId(), l.getMaxMemory(), l.getMaxMemory(),
        l.getMaxCoreUnits(), l.getMaxCoreUnits(), l.getMaxGpuMemory(), l.getMaxGpuMemory(),
        l.getMaxGpuUnits(), l.getMaxGpuUnits(), l.getThreads());
  }
  ...
  @Override
  public void insertLocalHostAssignment(HostInterface h, FrameInterface frame,
      LocalHostAssignment l) {
    l.id = SqlUtil.genKeyRandom();
  ...
  private static final String QUERY_FOR_LJA = "SELECT " + "pk_host_local," + "pk_job," + "pk_layer,"
      + "pk_frame," + "pk_host," + "int_mem_idle," + "int_mem_max," + "int_cores_idle,"
      + "int_cores_max," + "int_gpu_mem_idle," + "int_gpu_mem_max," + "int_gpus_idle,"
      + "int_gpus_max," + "int_threads, " + "str_type " + "FROM " + "host_local ";

  @Override
  public List<LocalHostAssignment> getLocalJobAssignment(HostInterface host) {
    return getJdbcTemplate().query(QUERY_FOR_LJA + "WHERE " + "host_local.pk_host = ? ", LJA_MAPPER,
        host.getHostId());
  }

  @Override
  public LocalHostAssignment getLocalJobAssignment(String id) {
    return getJdbcTemplate().queryForObject(QUERY_FOR_LJA + " WHERE pk_host_local = ?", LJA_MAPPER,
        id);
  }

  @Override
  public LocalHostAssignment getLocalJobAssignment(String hostId, String jobId) {
    return getJdbcTemplate().queryForObject(QUERY_FOR_LJA + " WHERE pk_host = ? and pk_job = ?",
        LJA_MAPPER, hostId, jobId);
  }

  @Override
  public boolean deleteLocalJobAssignment(LocalHostAssignment l) {
    return getJdbcTemplate().update("DELETE FROM host_local WHERE pk_host_local = ?",
        l.getId()) > 0;
  }

  private static final String HAS_LOCAL_JOB =
      "SELECT " + "COUNT(1) " + "FROM " + "host_local " + "WHERE " + "host_local.pk_host = ? ";
  ...
  public boolean hasActiveLocalJob(HostInterface host) {
    return getJdbcTemplate().queryForObject(HAS_ACTIVE_LOCAL_JOB, Integer.class,
        host.getHostId()) > 0;
  }
  ...
  public int getCoreUsageDifference(LocalHostAssignment l, int coreUnits) {
    return getJdbcTemplate().queryForObject(
        "SELECT ? - int_cores_max  FROM host_local WHERE pk_host_local=?", Integer.class, coreUnits,
        l.getId());
  }
  ...
  public int getGpuUsageDifference(LocalHostAssignment l, int gpuUnits) {
    return getJdbcTemplate().queryForObject(
        "SELECT ? - int_gpus_max FROM host_local WHERE pk_host_local=?", Integer.class, gpuUnits,
        l.getId());
  }

  private static final String UPDATE_MAX_CORES =
      "UPDATE " + "host_local " + "SET " + "int_cores_idle = int_cores_idle + (? - int_cores_max), "
          + "int_cores_max = ? " + "WHERE " + "pk_host_local = ? ";
  ...
  private static final String UPDATE_MAX_GPUS =
      "UPDATE " + "host_local " + "SET " + "int_gpus_idle = int_gpus_idle + (? - int_gpus_max), "
          + "int_gpus_max = ? " + "WHERE " + "pk_host_local = ? ";
  ...
  private static final String UPDATE_MAX_GPU_MEMORY = "UPDATE " + "host_local " + "SET "
      + "int_gpu_mem_idle = int_gpu_mem_idle + (? - int_gpu_mem_max), " + "int_gpu_mem_max = ? "
      + "WHERE " + "pk_host_local = ? ";

  @Override
  public boolean updateMaxGpuMemory(LocalHostAssignment l, long maxGpuMemory) {
    return getJdbcTemplate().update(UPDATE_MAX_GPU_MEMORY, maxGpuMemory, maxGpuMemory,
        l.getId()) > 0;
  }
  ...
  public boolean deactivate(LocalHostAssignment l) {
    return getJdbcTemplate().update(
        "UPDATE host_local SET b_active = false WHERE " + "pk_host_local = ? AND b_active = true",
        l.getId()) > 0;
  }
  ...
  public boolean allocateCoresFromHost(HostInterface h, int cores) {
    ...
    try {
      return getJdbcTemplate().update(
          "UPDATE host SET int_cores_idle = int_cores_idle - ? " + "WHERE pk_host = ?", cores,
          h.getHostId()) > 0;
    } catch (DataAccessException e) {
      throw new ResourceReservationFailureException(
          "Failed to allocate " + cores + " from host, " + e);
    }
  ...
  public boolean allocateGpusFromHost(HostInterface h, int gpus) {
    ...
    try {
      return getJdbcTemplate().update(
          "UPDATE host SET int_gpus_idle = int_gpus_idle - ? " + "WHERE pk_host = ?", gpus,
          h.getHostId()) > 0;
    } catch (DataAccessException e) {
      throw new ResourceReservationFailureException(
          "Failed to allocate " + gpus + " GPU from host, " + e);
    }
  }
  ...
  public boolean deallocateCoresFromHost(HostInterface h, int cores) {
    try {
      ...
      throw new ResourceReservationFailureException(
          "Failed to de-allocate " + cores + " from host, " + e);
    }
  }
  ...
  public boolean deallocateGpusFromHost(HostInterface h, int gpus) {
    try {
      ...
      throw new ResourceReservationFailureException(
          "Failed to de-allocate " + gpus + " GPU from host, " + e);
    }
  }
  ...
  public boolean hasResourceDeficit(HostInterface host) {
    return getJdbcTemplate().queryForObject(
        "SELECT COUNT(1) FROM host_local WHERE "
            ...
            + "int_mem_max < int_mem_max - int_mem_idle) AND " + "host_local.pk_host= ?",
