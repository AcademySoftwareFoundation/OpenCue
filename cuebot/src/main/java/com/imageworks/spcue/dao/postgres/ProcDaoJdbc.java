...
public class ProcDaoJdbc extends JdbcDaoSupport implements ProcDao {
  ...
  public boolean verifyRunningProc(String procId, String frameId) {
    try {
      String pk_frame = getJdbcTemplate().queryForObject(VERIFY_RUNNING_PROC, String.class, procId);
      if (pk_frame != null) {
  ...
  private static final String INSERT_VIRTUAL_PROC = "INSERT INTO " + "proc " + "( " + "pk_proc, "
      + "pk_host, " + "pk_show, " + "pk_layer," + "pk_job," + "pk_frame, " + "int_cores_reserved, "
      + "int_mem_reserved, " + "int_mem_pre_reserved, " + "int_mem_used, " + "int_gpus_reserved, "
      + "int_gpu_mem_reserved, " + "int_gpu_mem_pre_reserved, " + "int_gpu_mem_used, " + "b_local "
      + ") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
  ...
  public void insertVirtualProc(VirtualProc proc) {
    ...
    int result = 0;
    try {
      result = getJdbcTemplate().update(INSERT_VIRTUAL_PROC, proc.getProcId(), proc.getHostId(),
          proc.getShowId(), proc.getLayerId(), proc.getJobId(), proc.getFrameId(),
          proc.coresReserved, proc.memoryReserved, proc.memoryReserved, memReservedMin,
          proc.gpusReserved, proc.gpuMemoryReserved, proc.gpuMemoryReserved, memGpuReservedMin,
      ...
      /*
       * This means the frame is already running. If that is the case, don't delete it, just set
       * pk_frame to null or the orphaned proc handler will catch it.
       */
      throw new ResourceDuplicationFailureException(
          "The frame " + proc.getFrameId() + " is already assigned to a proc.");
    } catch (Exception e) {
      String msg =
          "unable to book proc " + proc.getName() + " on frame " + proc.getFrameId() + " , " + e;
      throw new ResourceReservationFailureException(msg, e);
    }

    if (result == 0) {
      String msg = "unable to book proc " + proc.id + " the insert query succeeded but returned 0";
      throw new ResourceReservationFailureException(msg);
    }
  }

  private static final String UPDATE_VIRTUAL_PROC_ASSIGN = "UPDATE " + "proc " + "SET "
      + "pk_show = ?, " + "pk_job = ?, " + "pk_layer = ?, " + "pk_frame = ?, "
      + "int_mem_used = 0, " + "int_mem_max_used = 0, " + "int_virt_used = 0, "
      + "int_virt_max_used = 0, " + "ts_dispatched = current_timestamp " + "WHERE " + "pk_proc = ?";
  ...
  public void updateVirtualProcAssignment(VirtualProc proc) {
    ...
    try {
      ...
      throw new ResourceDuplicationFailureException(
          "The frame " + proc.getFrameId() + " is already assigned to " + "the proc " + proc);
    } catch (Exception e) {
  ...
  public boolean clearVirtualProcAssignment(FrameInterface frame) {
    return getJdbcTemplate().update(CLEAR_VIRTUAL_PROC_ASSIGN_BY_FRAME, frame.getFrameId()) == 1;
  }
  ...
  @Override
  public void updateProcMemoryUsage(FrameInterface f, long rss, long maxRss, long vss, long maxVss,
      long usedGpuMemory, long maxUsedGpuMemory, long usedSwapMemory, byte[] children) {
    /*
     * This method is going to repeat for a proc every 1 minute, so if the proc is being touched by
     * another thread, then return quietly without updating memory usage.
     *
     * If another thread is accessing the proc record, that means the proc is probably being booked
     * to another frame, which makes this update invalid anyway.
     */
    try {
      if (getJdbcTemplate().queryForObject("SELECT pk_frame FROM proc WHERE pk_frame=? FOR UPDATE",
          String.class, f.getFrameId()).equals(f.getFrameId())) {

        getJdbcTemplate().update(new PreparedStatementCreator() {
          @Override
          public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
            PreparedStatement updateProc = conn.prepareStatement(UPDATE_PROC_MEMORY_USAGE);
      ...
      logger
          .info("The proc for frame " + f + " could not be updated with new memory stats: " + dae);
    }
  }
  ...
  private static final String GET_VIRTUAL_PROC = "SELECT " + "proc.pk_proc," + "proc.pk_host,"
      + "proc.pk_show," + "proc.pk_job," + "proc.pk_layer," + "proc.pk_frame," + "proc.b_unbooked,"
      + "proc.b_local," + "host.pk_alloc, " + "alloc.pk_facility," + "proc.int_cores_reserved,"
      + "proc.int_mem_reserved," + "proc.int_mem_max_used," + "proc.int_mem_used,"
      + "proc.int_gpus_reserved," + "proc.int_gpu_mem_reserved," + "proc.int_gpu_mem_max_used,"
      + "proc.int_gpu_mem_used," + "proc.bytea_children," + "proc.int_virt_max_used,"
      + "proc.int_virt_used," + "host.str_name AS host_name, "
      + "COALESCE(job.str_os, '') AS str_os " + "FROM " + "proc, " + "job, " + "host, "
      + "host_stat, " + "alloc " + "WHERE " + "proc.pk_host = host.pk_host " + "AND "
      + "host.pk_host = host_stat.pk_host " + "AND " + "host.pk_alloc = alloc.pk_alloc " + "AND "
  ...
  private static final String GET_VIRTUAL_PROC_LIST = "SELECT " + "proc.*, "
      + "host.str_name AS host_name, " + "host.pk_alloc, " + "COALESCE(job.str_os, '') AS str_os, "
      + "alloc.pk_facility " + "FROM " + "proc, " + "frame, " + "host," + "host_stat, " + "alloc, "
      + "layer," + "job, " + "folder, " + "show " + "WHERE " + "proc.pk_show = show.pk_show "
      + "AND " + "proc.pk_host = host.pk_host " + "AND " + "host.pk_alloc = alloc.pk_alloc "
      + "AND " + "host.pk_host = host_stat.pk_host " + "AND " + "proc.pk_job = job.pk_job " + "AND "
      + "proc.pk_layer = layer.pk_layer " + "AND " + "proc.pk_frame = frame.pk_frame " + "AND "
      + "job.pk_folder = folder.pk_folder ";

  public List<VirtualProc> findVirtualProcs(ProcSearchInterface r) {
    return getJdbcTemplate().query(r.getFilteredQuery(GET_VIRTUAL_PROC_LIST), VIRTUAL_PROC_MAPPER,
        r.getValuesArray());
  }
  ...
  private static final String FIND_VIRTUAL_PROCS_LJA = GET_VIRTUAL_PROC_LIST + "AND proc.pk_job=( "
      + "SELECT pk_job FROM host_local WHERE pk_host_local = ?) " + "AND proc.pk_host=("
  ...
  public boolean setRedirectTarget(ProcInterface p, Redirect r) {
    ...
    return getJdbcTemplate().update("UPDATE proc SET str_redirect=?, b_unbooked=? WHERE pk_proc=?",
        name, unbooked, p.getProcId()) == 1;
  }

  public void unbookProc(ProcInterface proc) {
    getJdbcTemplate().update("UPDATE proc SET b_unbooked=true WHERE pk_proc=?", proc.getProcId());
  }
  ...
  public String getCurrentJobId(ProcInterface p) {
    return getJdbcTemplate().queryForObject("SELECT pk_job FROM proc WHERE pk_proc=?", String.class,
        p.getProcId());
  }
  ...
  private static final String GET_ORPHANED_PROC_LIST = "SELECT " + "proc.*, "
      + "host.str_name AS host_name, " + "COALESCE(job.str_os, '') AS str_os, " + "host.pk_alloc, "
      + "alloc.pk_facility " + "FROM " + "proc, " + "host, " + "host_stat," + "alloc, " + "job "
      + "WHERE " + "proc.pk_host = host.pk_host " + "AND " + "host.pk_host = host_stat.pk_host "
      + "AND " + "host.pk_alloc = alloc.pk_alloc " + "AND " + "job.pk_job = proc.pk_job " + "AND "
  ...
  public boolean increaseReservedMemory(ProcInterface p, long value) {
    try {
      return getJdbcTemplate().update(
          "UPDATE proc SET int_mem_reserved=? WHERE pk_proc=? AND int_mem_reserved < ?", value,
          p.getProcId(), value) == 1;
    } catch (Exception e) {
      // check by trigger erify_host_resources
      throw new ResourceReservationFailureException(
          "failed to increase memory reservation for proc " + p.getProcId() + " to " + value
              + ", proc does not have that much memory to spare.");
    }
  }
  ...
  public long getReservedGpuMemory(ProcInterface proc) {
    return getJdbcTemplate().queryForObject("SELECT int_gpu_mem_reserved FROM proc WHERE pk_proc=?",
        Long.class, proc.getProcId());
  }

  private static final String FIND_UNDERUTILIZED_PROCS =
      "SELECT " + "proc.pk_proc," + "proc.int_mem_reserved - layer_mem.int_max_rss AS free_mem "
          + "FROM " + "proc," + "host, " + "layer_mem " + "WHERE " + "proc.pk_host = host.pk_host "
          + "AND " + "proc.pk_layer = layer_mem.pk_layer " + "AND " + "layer_mem.int_max_rss > 0 "
          + "AND " + "host.pk_host = ? " + "AND " + "proc.pk_proc != ? " + "AND "
  ...
  public boolean balanceUnderUtilizedProcs(ProcInterface targetProc, long targetMem) {
    ...
    if (memBorrowedTotal < targetMem) {
      logger.info(
          "mem borrowed " + memBorrowedTotal + " was less than the target memory of " + targetMem);
      return false;
    }

    /*
     * This might fail... I'm not really sure if we should fail the whole operation or what. Just
     * gonna let it ride for now.
     */
    for (Map.Entry<String, Long> set : borrowMap.entrySet()) {
      int success = getJdbcTemplate().update(
          "UPDATE proc SET int_mem_reserved = int_mem_reserved - ? WHERE pk_proc=?", set.getValue(),
          set.getKey());
  ...
  /**
   * Updates proc counts for the host, subscription, layer, job, folder, and proc point when a proc
   * is destroyed.
   *
   * @param proc
   */
    ...
    if (!proc.isLocalDispatch) {
      ...
      getJdbcTemplate().update(
          "UPDATE " + "point " + "SET " + "int_cores = int_cores - ?, " + "int_gpus = int_gpus - ? "
              + "WHERE " + "pk_dept = " + "(SELECT pk_dept FROM job WHERE pk_job=?) " + "AND "
    ...
    if (proc.isLocalDispatch) {
      ...
      getJdbcTemplate().update(
          "UPDATE " + "host_local " + "SET " + "int_cores_idle = int_cores_idle + ?, "
              + "int_mem_idle = int_mem_idle + ?, " + "int_gpus_idle = int_gpus_idle + ?, "
              + "int_gpu_mem_idle = int_gpu_mem_idle + ? " + "WHERE " + "pk_job = ? " + "AND "
              + "pk_host = ? ",
          proc.coresReserved, proc.memoryReserved, proc.gpusReserved, proc.gpuMemoryReserved,
          proc.getJobId(), proc.getHostId());
    }
  }
