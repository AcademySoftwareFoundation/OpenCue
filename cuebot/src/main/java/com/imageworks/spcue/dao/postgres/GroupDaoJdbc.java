...
public class GroupDaoJdbc extends JdbcDaoSupport implements GroupDao {
  ...
  public String getRootGroupId(ShowInterface show) {
    return getJdbcTemplate().queryForObject(
        "SELECT pk_folder FROM folder WHERE pk_show=? AND pk_parent_folder IS NULL", String.class,
        show.getShowId());
  }
  ...
  public void deleteGroup(GroupInterface group) {
    ...
    if (childGroupCount(group) > 0) {
      throw new EntityRemovalError(
          "failed to delete group " + group.getName() + ", still has sub groups");
    }

    if (childJobCount(group) > 0) {
      throw new EntityRemovalError(
          "failed to delete group " + group.getName() + ", still has sub jobs");
    }

    // reparent all jobs to root group
    getJdbcTemplate().update("UPDATE job SET pk_folder=? WHERE pk_folder=?", getRootGroupId(group),
        group.getId());
  ...
  public static final String INSERT_GROUP = "INSERT INTO " + "folder " + "( " + "pk_folder,"
      + "pk_parent_folder," + "pk_show, " + "pk_dept," + "str_name " + ") " + "VALUES (?,?,?,?,?)";
  ...
  public void insertGroup(GroupDetail group) {
    ...
    try {
      getJdbcTemplate().update(INSERT_GROUP, group.id, parentId, group.showId, group.deptId,
          group.name);
    } catch (Exception e) {
  ...
  public void updateGroupParent(GroupInterface group, GroupInterface dest) {
    ...
    if (group.getGroupId().equals(dest.getGroupId())) {
      throw new EntityModificationError("error moving group, " + "cannot move group into itself");
    }

    if (!group.getShowId().equals(dest.getShowId())) {
      throw new EntityModificationError(
          "error moving group, " + "cannot move groups between shows");
    }
    ...
    while (true) {
      destParent = getJdbcTemplate().queryForObject(
          "SELECT pk_parent_folder FROM folder WHERE pk_folder=?", String.class, destParent);
      if (destParent == null) {
        break;
      }
      if (destParent.equals(group.getGroupId())) {
        throw new EntityModificationError(
            "error moving group, you cannot move a group " + "into one of its sub groups");
      }
      recurse++;
    ...
    if (result == 0) {
      throw new EntityModificationError("error moving group, " + group.getName()
          + ", the group does not exist or its the top level group");
    }
  }

  @Override
  public void updateName(GroupInterface group, String value) {
    getJdbcTemplate().update("UPDATE folder SET str_name=? WHERE pk_folder=?", value,
        group.getId());
  }

  @Override
  public void updateDepartment(GroupInterface group, DepartmentInterface dept) {
    getJdbcTemplate().update("UPDATE folder SET pk_dept=? WHERE pk_folder=?",
  ...
  public void updateDefaultJobMaxCores(GroupInterface group, int value) {
    ...
    if (value < CueUtil.ONE_CORE && value != CueUtil.FEATURE_DISABLED) {
      String msg = "The default max cores for a job must " + "be greater than a single core";
      throw new IllegalArgumentException(msg);
    }
    getJdbcTemplate().update("UPDATE folder SET int_job_max_cores=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateDefaultJobMinCores(GroupInterface group, int value) {
    ...
    if (value < CueUtil.ONE_CORE && value != CueUtil.FEATURE_DISABLED) {
      String msg = "The default min cores for a job must " + "be greater than a single core";
      throw new IllegalArgumentException(msg);
    }
    getJdbcTemplate().update("UPDATE folder SET int_job_min_cores=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateMaxCores(GroupInterface group, int value) {
    ...
    if (value < CueUtil.ONE_CORE && value != CueUtil.FEATURE_DISABLED) {
      String msg =
          "The group max cores feature must " + "be a whole core or greater, pass in: " + value;
      throw new IllegalArgumentException(msg);
    }

    getJdbcTemplate().update("UPDATE folder_resource SET int_max_cores=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateMinCores(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder_resource SET int_min_cores=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public boolean isOverMinCores(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_OVER_MIN_CORES, Integer.class, job.getJobId()) > 0;
  }
  ...
  public void updateDefaultJobMaxGpus(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder SET int_job_max_gpus=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateDefaultJobMinGpus(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder SET int_job_min_gpus=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateMaxGpus(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder_resource SET int_max_gpus=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateMinGpus(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder_resource SET int_min_gpus=? WHERE pk_folder=?", value,
        group.getId());
  }
  ...
  public void updateDefaultJobPriority(GroupInterface group, int value) {
    ...
    getJdbcTemplate().update("UPDATE folder SET int_job_priority=? WHERE pk_folder=?", value,
        group.getId());
    if (value != CueUtil.FEATURE_DISABLED) {
      getJdbcTemplate().update("UPDATE job_resource SET int_priority=? WHERE pk_job IN ("
          + "SELECT pk_job from job WHERE pk_folder=?)", value, group.getId());
    }
  }

  private static final String GET_GROUP_DETAIL =
      "SELECT " + "folder.pk_folder, " + "folder.int_job_max_cores," + "folder.int_job_min_cores,"
          + "folder.int_job_max_gpus," + "folder.int_job_min_gpus," + "folder.int_job_priority,"
          + "folder.str_name," + "folder.pk_parent_folder," + "folder.pk_show," + "folder.pk_dept,"
  ...
  public GroupDetail getGroupDetail(String id) {
    return getJdbcTemplate().queryForObject(GET_GROUP_DETAIL + " AND folder.pk_folder=?",
        GROUP_DETAIL_MAPPER, id);
  }

  @Override
  public GroupDetail getGroupDetail(JobInterface job) {
    return getJdbcTemplate().queryForObject(GET_GROUP_DETAIL_BY_JOB, GROUP_DETAIL_MAPPER,
        job.getId());
  }
  ...
  public GroupInterface getGroup(String id) {
    return getJdbcTemplate().queryForObject(
        "SELECT pk_show, pk_folder,str_name FROM folder WHERE pk_folder=?", GROUP_MAPPER, id);
  }

  @Override
  public List<GroupInterface> getGroups(List<String> idl) {
    return getJdbcTemplate().query("SELECT pk_show, pk_folder, str_name FROM folder WHERE  "
        + SqlUtil.buildBindVariableArray("pk_folder", idl.size()), GROUP_MAPPER, idl.toArray());
  }
  ...
  public List<GroupInterface> getChildren(GroupInterface group) {
    return getJdbcTemplate().query(
        "SELECT pk_show, pk_folder, str_name FROM folder WHERE pk_parent_folder = ?", GROUP_MAPPER,
        group.getGroupId());
  }

  private static final String IS_MANAGED = "SELECT " + "COUNT(1) " + "FROM " + "folder, " + "point "
      + "WHERE " + "folder.pk_show = point.pk_show " + "AND " + "folder.pk_dept = point.pk_dept "
      + "AND " + "folder.b_exclude_managed = false " + "AND " + "point.b_managed = true " + "AND "
      + "folder.pk_folder = ?";

  @Override
  public boolean isManaged(GroupInterface group) {
    return getJdbcTemplate().queryForObject(IS_MANAGED, Integer.class, group.getGroupId()) > 0;
  }

  public static final RowMapper<GroupInterface> GROUP_MAPPER = new RowMapper<GroupInterface>() {
  ...
  public static final RowMapper<GroupDetail> GROUP_DETAIL_MAPPER = new RowMapper<GroupDetail>() {
  ...
  private int childGroupCount(GroupInterface group) {
    return getJdbcTemplate().queryForObject("SELECT COUNT(*) FROM folder WHERE pk_parent_folder=?",
        Integer.class, group.getId());
  }

  private int childJobCount(GroupInterface group) {
    return getJdbcTemplate().queryForObject(
        "SELECT COUNT(*) FROM job WHERE pk_folder=? AND str_state=?", Integer.class, group.getId(),
