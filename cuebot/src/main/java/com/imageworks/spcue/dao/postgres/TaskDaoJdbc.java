...
public class TaskDaoJdbc extends JdbcDaoSupport implements TaskDao {
  ...
  public boolean isManaged(TaskInterface t) {
    try {
      return getJdbcTemplate().queryForObject(
          "SELECT b_managed FROM point WHERE pk_show=? and pk_dept=?", Integer.class, t.getShowId(),
          t.getDepartmentId()) == 1;
  ...
  public void insertTask(TaskEntity task) {
    ...
    getJdbcTemplate().update(INSERT_TASK, task.id, task.getPointId(), task.shot, task.minCoreUnits);
  }

  private static final String GET_TASK_DETAIL =
      "SELECT " + "point.pk_dept," + "point.pk_show," + "point.pk_point," + "task.pk_task,"
  ...
  public TaskEntity getTaskDetail(DepartmentInterface d, String shot) {
    return getJdbcTemplate().queryForObject(
        GET_TASK_DETAIL + " AND point.pk_dept = ? AND task.str_shot = ?", TASK_DETAIL_MAPPER,
  ...
  public TaskEntity getTaskDetail(JobInterface j) {
    ...
    return getJdbcTemplate().queryForObject(
        GET_TASK_DETAIL + " AND task.str_shot = ? AND point.pk_dept = ?", TASK_DETAIL_MAPPER,
        map.get("str_shot").toString(), map.get("pk_dept").toString());
  }
  ...
  public void adjustTaskMinCores(TaskInterface t, int value) {
    ...
    getJdbcTemplate().update("UPDATE task SET int_adjust_cores = ? - int_min_cores WHERE pk_task=?",
        value, t.getTaskId());
  }
  ...
  public void mergeTask(TaskEntity t) {
    ...
    if (t.minCoreUnits == 0) {
      getJdbcTemplate().update("DELETE FROM task WHERE pk_point=? AND str_shot=? ", t.getPointId(),
          t.shot);
  ...
  public void clearTaskAdjustments(PointInterface cdept) {
    getJdbcTemplate().update(CLEAR_TASK_ADJUSTMENTS, cdept.getShowId(), cdept.getDepartmentId());
  }
  ...
  private static final String IS_JOB_MANAGED = "SELECT " + "COUNT(1) " + "FROM " + "job," + "task,"
      + "point " + "WHERE " + "job.pk_show = point.pk_show " + "AND "
