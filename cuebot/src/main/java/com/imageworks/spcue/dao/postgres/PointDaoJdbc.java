...
public class PointDaoJdbc extends JdbcDaoSupport implements PointDao {
  ...
  public void insertPointConf(PointDetail t) {
    ...
    getJdbcTemplate().update("INSERT INTO point (pk_point, pk_show, pk_dept) VALUES (?,?,?)", t.id,
        t.getShowId(), t.getDepartmentId());
  }
  ...
  public PointDetail insertPointConf(ShowInterface show, DepartmentInterface dept) {
    ...
    getJdbcTemplate().update("INSERT INTO point (pk_point, pk_show, pk_dept) VALUES (?,?,?)", r.id,
        r.getShowId(), r.getDepartmentId());
    return r;
  }
  ...
  public boolean pointConfExists(ShowInterface show, DepartmentInterface dept) {
    return getJdbcTemplate().queryForObject(
        "SELECT COUNT(1) FROM point WHERE pk_show=? AND pk_dept=?", Integer.class, show.getShowId(),
        dept.getDepartmentId()) > 0;
  }

  private static final String UPDATE_TI_MANAGED = "UPDATE " + "point " + "SET "
      + "b_managed = true," + "str_ti_task=?, " + "int_min_cores=? " + "WHERE " + "pk_point=?";
  ...
  private static final String UPDATE_DISABLE_TI_MANAGED = "UPDATE " + "point " + "SET "
      + "b_managed = false," + "str_ti_task=null, " + "int_min_cores=0 " + "WHERE " + "pk_point=?";
  ...
  private static final String GET_POINT_CONFIG_DETAIL_BY_SHOW_DEPT =
      "SELECT " + "pk_point," + "pk_dept," + "pk_show," + "str_ti_task," + "int_min_cores, "
          + "b_managed " + "FROM " + "point " + "WHERE " + "pk_show = ? " + "AND " + "pk_dept = ? ";
  ...
  private static final String IS_OVER_MIN_CORES = "SELECT " + "COUNT(1) " + "FROM " + "job,"
      + "point p " + "WHERE " + "job.pk_show = p.pk_show " + "AND " + "job.pk_dept = p.pk_dept "
      + "AND " + "p.int_cores > p.int_min_cores " + "AND " + "job.pk_job = ?";
