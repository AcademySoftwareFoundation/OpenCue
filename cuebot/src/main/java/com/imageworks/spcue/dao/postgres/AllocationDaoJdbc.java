...
public class AllocationDaoJdbc extends JdbcDaoSupport implements AllocationDao {
  ...
  private static final String GET_ALLOCATION = "SELECT " + "alloc.pk_facility," + "alloc.pk_alloc, "
      + "alloc.str_name, " + "alloc.str_tag, " + "facility.str_name AS facility_name " + "FROM "
      + "alloc, " + "facility " + "WHERE " + "alloc.pk_facility = facility.pk_facility ";

  public AllocationEntity getAllocationEntity(String id) {
    return getJdbcTemplate().queryForObject(GET_ALLOCATION + " AND pk_alloc=?", ALLOC_MAPPER, id);
  }

  public AllocationEntity findAllocationEntity(String facility, String name) {
    return getJdbcTemplate().queryForObject(GET_ALLOCATION + " AND alloc.str_name=?", ALLOC_MAPPER,
        String.format("%s.%s", facility, name));
  }
  ...
  public void insertAllocation(FacilityInterface facility, AllocationEntity detail) {
    ...
    String new_alloc_name = String.format("%s.%s", facility.getName(), detail.getName());
    /*
     * Checks if the allocation already exits.
     */
    if (getJdbcTemplate().queryForObject("SELECT COUNT(1) FROM alloc WHERE str_name=?",
        Integer.class, new_alloc_name) > 0) {

      getJdbcTemplate().update("UPDATE alloc SET b_enabled = true WHERE str_name=?",
          new_alloc_name);
    } else {
      detail.id = SqlUtil.genKeyRandom();
      detail.name = new_alloc_name;
      getJdbcTemplate().update(INSERT_ALLOCATION, detail.id, facility.getFacilityId(), detail.name,
          detail.tag);
    }
  }

  public void deleteAllocation(AllocationInterface a) {
    if (getJdbcTemplate().queryForObject("SELECT COUNT(1) FROM host WHERE pk_alloc=?",
        Integer.class, a.getAllocationId()) > 0) {
      throw new EntityRemovalError("allocation still contains hosts", a);
    }

    if (getJdbcTemplate().queryForObject("SELECT b_default FROM alloc WHERE pk_alloc=?",
        Boolean.class, a.getAllocationId())) {
    ...
    /*
     * Allocations are logged in historical data so once they are used you can't specifically delete
     * them. They are disabled instead.
     */
    try {
      getJdbcTemplate().update("DELETE FROM alloc WHERE pk_alloc=?", a.getAllocationId());
  ...
  public void updateAllocationName(AllocationInterface a, String name) {
    if (!Pattern.matches("^\\w+$", name)) {
      throw new IllegalArgumentException(
          "The new allocation name" + "must be alpha numeric and not contain the facility prefix.");
    }

    String[] parts = a.getName().split("\\.", 2);
    String new_name = String.format("%s.%s", parts[0], name);

    getJdbcTemplate().update("UPDATE alloc SET str_name=? WHERE pk_alloc=?", new_name,
        a.getAllocationId());
  }

  public void updateAllocationTag(AllocationInterface a, String tag) {
    getJdbcTemplate().update("UPDATE alloc SET str_tag=? WHERE pk_alloc=?", tag,
        a.getAllocationId());

    getJdbcTemplate().update(
        "UPDATE host_tag SET str_tag=? WHERE " + "host_tag.str_tag_type='Alloc' AND pk_host IN "
            + "(SELECT pk_host FROM host WHERE host.pk_alloc=?)",
        tag, a.getAllocationId());

    for (Map<String, Object> e : getJdbcTemplate()
        .queryForList("SELECT pk_host FROM host WHERE pk_alloc=?", a.getAllocationId())) {
  ...
  public AllocationEntity getDefaultAllocationEntity() {
    return getJdbcTemplate().queryForObject(GET_ALLOCATION + " AND alloc.b_default = true LIMIT 1",
        ALLOC_MAPPER);
  }

  @Override
  public void updateAllocationBillable(AllocationInterface alloc, boolean value) {
    getJdbcTemplate().update("UPDATE alloc SET b_billable = ? WHERE pk_alloc = ?", value,
