...
/**
 * A DAO class for loading Filters, Actions, and Matchers. Part of the job filtering system.
 *
 * @category DAO
 */
public class FilterDaoJdbc extends JdbcDaoSupport implements FilterDao {
  ...
  private static final String GET_FILTERS = "SELECT " + "filter.* " + "FROM " + "filter " + "WHERE "
      + "pk_show=? " + "ORDER BY " + "f_order ASC";
  ...
  public List<FilterEntity> getActiveFilters(ShowInterface show) {
    return getJdbcTemplate().query(GET_ACTIVE_FILTERS, FILTER_DETAIL_MAPPER, show.getShowId());
  }

  public List<FilterEntity> getFilters(ShowInterface show) {
    return getJdbcTemplate().query(GET_FILTERS, FILTER_DETAIL_MAPPER, show.getShowId());
  }

  public void deleteFilter(FilterInterface f) {
    getJdbcTemplate().update("DELETE FROM action WHERE pk_filter=?", f.getFilterId());
    getJdbcTemplate().update("DELETE FROM matcher WHERE pk_filter=?", f.getFilterId());
    getJdbcTemplate().update("DELETE FROM filter WHERE pk_filter=?", f.getFilterId());
    reorderFilters(f);
  }
  ...
  public void insertFilter(FilterEntity f) {
    ...
    getJdbcTemplate().update(INSERT_FILTER, f.id, f.getShowId(), f.name, f.type.toString(),
        f.getShowId());
    reorderFilters(f);
  }

  public void updateSetFilterEnabled(FilterInterface f, boolean enabled) {
    getJdbcTemplate().update("UPDATE filter SET b_enabled=? WHERE pk_filter=?", enabled,
        f.getFilterId());
  }

  public void updateSetFilterName(FilterInterface f, String name) {
    getJdbcTemplate().update("UPDATE filter SET str_name=? WHERE pk_filter=?", name,
        f.getFilterId());
  }

  public void updateSetFilterOrder(FilterInterface f, double order) {
    getJdbcTemplate().update("UPDATE filter SET f_order=? - 0.1 WHERE pk_filter=?", order,
        f.getFilterId());
    reorderFilters(f);
  }
  ...
  public void lowerFilterOrder(FilterInterface f, int by) {
    ...
    getJdbcTemplate().update("UPDATE filter SET f_order=f_order + ? WHERE pk_filter=?", lower_by,
        f.getFilterId());
    reorderFilters(f);
  }
  ...
  public void raiseFilterOrder(FilterInterface f, int by) {
    ...
    getJdbcTemplate().update("UPDATE filter SET f_order=f_order + ? WHERE pk_filter=?", raise_by,
        f.getFilterId());
    reorderFilters(f);
  }

  public void updateSetFilterType(FilterInterface f, FilterType type) {
    getJdbcTemplate().update("UPDATE filter SET str_type=? WHERE pk_filter=?", type.toString(),
        f.getFilterId());
  }
  ...
  public FilterEntity findFilter(ShowInterface show, String name) {
    return getJdbcTemplate().queryForObject(GET_FILTER + " WHERE pk_show=? AND str_name=?",
        FILTER_DETAIL_MAPPER, show.getShowId(), name);
  }

  public FilterEntity getFilter(String id) {
    return getJdbcTemplate().queryForObject(GET_FILTER + " WHERE pk_filter=?", FILTER_DETAIL_MAPPER,
        id);
  }
