...
public class DispatcherDaoJdbc extends JdbcDaoSupport implements DispatcherDao {
  ...
  public DispatcherDaoJdbc(Environment env) {
    this.schedulingMode = SchedulingMode
        .valueOf(env.getProperty("dispatcher.scheduling_mode", String.class, "PRIORITY_ONLY"));
  }
  ...
  private List<SortableShow> getBookableShows(AllocationInterface alloc) {
    ...
    if (cached == null) {
      bookableShows.put(key,
          new ShowCache(getJdbcTemplate().query(FIND_SHOWS, SHOW_MAPPER, alloc.getAllocationId())));
  ...
  private Set<String> findDispatchJobs(DispatchHost host, int numJobs, boolean shuffleShows) {
    ...
    for (SortableShow s : shows) {
      ...
      /**
       * Check if the show is over its subscription because we're using cached SortableShows, we
       * don't pull a fresh list of shows for a while. If the show is over its subscription the
       * alloc gets add to the SortableShow skipped alloc set.
       */
  ...
  public List<DispatchFrame> findNextDispatchFrames(JobInterface job, DispatchHost host,
    ...
    if (host.isLocalDispatch) {
      frames = getJdbcTemplate().query(FIND_LOCAL_DISPATCH_FRAME_BY_JOB_AND_HOST,
          FrameDaoJdbc.DISPATCH_FRAME_MAPPER, host.idleMemory, host.idleGpuMemory, job.getJobId(),
          limit);
  ...
  public List<DispatchFrame> findNextDispatchFrames(LayerInterface layer, VirtualProc proc,
    ...
    if (proc.isLocalDispatch) {
      ...
      frames = getJdbcTemplate().query(FIND_DISPATCH_FRAME_BY_LAYER_AND_PROC,
          FrameDaoJdbc.DISPATCH_FRAME_MAPPER, proc.coresReserved, proc.memoryReserved,
          proc.gpusReserved, proc.gpuMemoryReserved, layer.getLayerId(), layer.getLayerId(),
          proc.hostName, limit);
    }
  ...
  public List<DispatchFrame> findNextDispatchFrames(LayerInterface layer, DispatchHost host,
    ...
    if (host.isLocalDispatch) {
      ...
      frames = getJdbcTemplate().query(FIND_DISPATCH_FRAME_BY_LAYER_AND_HOST,
          FrameDaoJdbc.DISPATCH_FRAME_MAPPER, host.idleCores, host.idleMemory,
          threadMode(host.threadMode), host.idleGpus, host.idleGpuMemory, layer.getLayerId(),
          layer.getLayerId(), host.getName(), limit);
    }
  ...
  public boolean findUnderProcedJob(JobInterface excludeJob, VirtualProc proc) {
    ...
    try {
      return getJdbcTemplate().queryForObject(FIND_UNDER_PROCED_JOB_BY_FACILITY, Integer.class,
          excludeJob.getShowId(), proc.getFacilityId(), proc.os, excludeJob.getShowId(),
          proc.getFacilityId(), proc.os, proc.coresReserved, proc.memoryReserved, proc.gpusReserved,
          proc.gpuMemoryReserved, proc.hostName) > 0;
  ...
  public boolean higherPriorityJobExists(JobDetail baseJob, VirtualProc proc) {
    long start = System.currentTimeMillis();
    try {
      return getJdbcTemplate().queryForObject(HIGHER_PRIORITY_JOB_BY_FACILITY_EXISTS, Boolean.class,
          baseJob.priority, proc.getFacilityId(), proc.os, proc.getFacilityId(), proc.os,
