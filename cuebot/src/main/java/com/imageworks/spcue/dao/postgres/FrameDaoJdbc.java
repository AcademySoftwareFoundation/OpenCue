...
public class FrameDaoJdbc extends JdbcDaoSupport implements FrameDao {
  ...
  private static final String UPDATE_FRAME_STOPPED_NORSS = "UPDATE " + "frame " + "SET "
      ...
      + "frame.pk_frame = ? " + "AND " + "frame.str_state = ? " + "AND " + "frame.int_version = ? ";
  ...
  private static final String UPDATE_FRAME_STOPPED = "UPDATE " + "frame " + "SET " + "str_state=?, "
      + "int_exit_status = ?, " + "ts_stopped = current_timestamp + interval '1' second, "
      + "ts_updated = current_timestamp, " + "int_mem_max_used = ?, "
      + "int_version = int_version + 1, " + "int_total_past_core_time = int_total_past_core_time + "
  ...
  private static final String UPDATE_FRAME_REASON = "UPDATE " + "frame " + "SET "
      ...
      + "frame.pk_frame = ? " + "AND " + "frame.pk_frame NOT IN " + "(SELECT proc.pk_frame FROM "
  ...
  private static final String UPDATE_FRAME_MEMORY_ERROR =
      "UPDATE " + "frame " + "SET " + "int_exit_status = ?, " + "int_version = int_version + 1 "
  ...
  private static final String UPDATE_FRAME_STARTED = "UPDATE " + "frame " + "SET "
      ...
      + "int_version = int_version + 1 " + "WHERE " + "pk_frame = ? " + "AND " + "str_state = ? "
      + "AND " + "int_version = ? " + "AND " + "frame.pk_layer IN (" + "SELECT " + "layer.pk_layer "
      + "FROM " + "layer " + "LEFT JOIN layer_limit ON layer_limit.pk_layer = layer.pk_layer "
  ...
  private static final String UPDATE_FRAME_RETRIES =
      "UPDATE " + "frame " + "SET " + "int_retries = int_retries + 1 " + "WHERE " + "pk_frame = ? "
  ...
  public void updateFrameStarted(VirtualProc proc, FrameInterface frame) {
    ...
    try {
      int result = getJdbcTemplate().update(UPDATE_FRAME_STARTED, FrameState.RUNNING.toString(),
    ...
    /*
     * Frames that were killed via nimby or hardware errors not attributed to the software do not
     * increment the retry counter. Like failed launch, orphaned frame, failed kill or down host.
     */
    try {
  ...
  private static final String UPDATE_FRAME_FIXED =
      "UPDATE " + "frame " + "SET " + "str_state = ?," + "str_host=?, " + "int_cores=?, "
          ...
          + "ts_stopped = null, " + "int_version = int_version + 1 " + "WHERE " + "pk_frame = ? "
  ...
  private static final String GET_DISPATCH_FRAME = "SELECT " + "show.str_name AS show_name, "
      ...
      + "job.str_name," + "job.str_shot," + "job.str_user," + "job.int_uid," + "job.str_log_dir,"
      + "COALESCE(str_os, '') AS str_os, " + "frame.str_name AS frame_name, "
      ...
      + "layer.str_range, " + "layer.int_chunk_size, " + "layer.str_services " + "FROM " + "layer, "
      + "job, " + "show, " + "frame LEFT JOIN proc ON (proc.pk_frame = frame.pk_frame) " + "WHERE "
  ...
  private static final String GET_FRAME_DETAIL =
      "SELECT " + "frame.*, " + "job.pk_facility," + "job.pk_show " + "FROM " + "frame," + "layer,"
          + "job," + "show " + "WHERE " + "frame.pk_job = job.pk_job " + "AND "
          + "frame.pk_layer = layer.pk_layer " + "AND " + "job.pk_show = show.pk_show ";

  private static final String GET_MINIMAL_FRAME = "SELECT " + "frame.pk_frame," + "frame.str_name, "
      + "frame.pk_job, " + "frame.pk_layer, " + "frame.str_state, " + "frame.int_version, "
      + "job.pk_show, " + "job.pk_facility " + "FROM " + "frame," + "layer," + "job," + "show "
      + "WHERE " + "frame.pk_job = job.pk_job " + "AND " + "frame.pk_layer = layer.pk_layer "
  ...
  public static final String FIND_ORPHANED_FRAMES =
      "SELECT " + "frame.pk_frame, " + "frame.pk_layer, " + "frame.str_name, "
          + "frame.int_version, " + "job.pk_job, " + "job.pk_show, " + "job.pk_facility " + "FROM "
          + "frame, " + "job " + "WHERE " + "job.pk_job = frame.pk_job " + "AND "
          + "frame.str_state = 'RUNNING' " + "AND " + "job.str_state = 'PENDING' " + "AND "
  ...
  public void insertFrames(LayerDetail layer, List<Integer> frames) {
    ...
    for (int frame : frames) {
      getJdbcTemplate().update(INSERT_FRAME, SqlUtil.genKeyRandom(), layer.getLayerId(),
          layer.getJobId(), CueUtil.buildFrameName(layer, frame), FrameState.SETUP.toString(),
          frame, count, layer.dispatchOrder);
      count++;
    }
  }
  ...
  public FrameDetail findFrameDetail(JobInterface job, String name) {
    ...
    return getJdbcTemplate().queryForObject(
        GET_FRAME_DETAIL + " AND frame.str_name=? AND frame.pk_job=?", FRAME_DETAIL_MAPPER, name,
        job.getJobId());
  }
  ...
  private static final String FIND_LONGEST_FRAME = "SELECT " + "pk_frame " + "FROM " + "frame, "
      + "layer " + "WHERE " + "frame.pk_layer = layer.pk_layer " + "AND " + "frame.pk_job = ? "
      + "AND " + "str_state=? " + "AND " + "layer.str_type=? " + "ORDER BY "
      + "ts_stopped - ts_started DESC " + "LIMIT 1";
  ...
  private static final String FIND_SHORTEST_FRAME = "SELECT " + "pk_frame " + "FROM " + "frame, "
      + "layer " + "WHERE " + "frame.pk_layer = layer.pk_layer " + "AND " + "frame.pk_job = ? "
      + "AND " + "frame.str_state = ? " + "AND " + "layer.str_type = ? " + "ORDER BY "
      + "ts_stopped - ts_started ASC " + "LIMIT 1";
  ...
  private static final String UPDATE_FRAME_STATE = "UPDATE " + "frame " + "SET " + "str_state = ?, "
      + "ts_updated = current_timestamp, " + "int_version = int_version + 1 " + "WHERE "
      + "pk_frame = ? " + "AND " + "int_version = ? ";
  ...
  private static final String MARK_AS_WAITING = "UPDATE " + "frame " + "SET " + "str_state=?, "
      ...
      + "int_depend_count = 0, " + "int_version = int_version + 1 " + "WHERE " + "pk_frame = ? "
      + "AND " + "int_version = ? " + "AND " + "str_state = ? ";
  ...
  private static final String MARK_AS_DEPEND =
      "UPDATE " + "frame " + "SET " + "str_state=?, " + "int_depend_count = ?, "
          + "ts_updated = current_timestamp, " + "int_version = int_version + 1 " + "WHERE "
          + "pk_frame = ? " + "AND " + "int_version = ? " + "AND " + "str_state = ? ";

  private static final String GET_FRAME_DEPEND_COUNT = "SELECT " + "COUNT(1) " + "FROM " + "depend "
      + "WHERE " + "( " + "(pk_job_depend_er = ? AND str_type LIKE 'JOB#_ON%' ESCAPE '#') " + "OR "
  ...
  public void reorderFramesFirst(LayerInterface layer, FrameSet frameSet) {
    ...
    for (int frameIdx = 0; frameIdx < size; frameIdx++) {
      ...
      logger.info(
          "reordering " + CueUtil.buildFrameName(layer, frameSet.get(frameIdx)) + " to " + start);
      start++;
    }
  }
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        int a = getJdbcTemplate().queryForObject(
            ...
            Integer.class, CueUtil.buildFrameName(layer, frameSet.get(i)), layer.getJobId(),
            layer.getLayerId());
        ...
        frames.add(
            new Object[] {b, layer.getLayerId(), CueUtil.buildFrameName(layer, frameSet.get(i))});
  ...
  public void staggerLayer(LayerInterface layer, String frameRange, int stagger) {
     ...
     */
    if (getJdbcTemplate().queryForObject("SELECT int_total_count FROM layer_stat WHERE pk_layer=?",
        Integer.class, layer.getLayerId()) == 1) {
      return;
    }
    ...
    /*
     * Find the dispatch order of the first frame we're working with and base our other staggers of
     * this value.
     */
    ...
    for (int i = 0; i < size; i++) {
      ...
      getJdbcTemplate().update(
          "UPDATE frame SET int_dispatch_order=? WHERE pk_layer=? and str_name=?", newDispatchOrder,
          layer.getLayerId(), CueUtil.buildFrameName(layer, frame));
    }
  }
  ...
  public boolean isFrameComplete(FrameInterface f) {
    ...
    String state = getJdbcTemplate().queryForObject("SELECT str_state FROM frame WHERE pk_frame=?",
  ...
  public ResourceUsage getResourceUsage(FrameInterface f) {
    /*
     * Using current_timestamp = ts_started here because ts_stopped is not set. Stopping the frame
     * allows it to be dispatched again, which could blow away the ts_stopped time.
     */
    return getJdbcTemplate().queryForObject(
        "SELECT " + "COALESCE(interval_to_seconds(current_timestamp - ts_started), 1) "
            + "AS int_clock_time, " + "COALESCE(int_cores, 100) AS int_cores," + "int_gpus "
            + "FROM " + "frame " + "WHERE " + "pk_frame = ?",
        RESOURCE_USAGE_MAPPER, f.getFrameId());
  }

  private static final String UPDATE_FRAME_MEMORY_USAGE_AND_LLU_TIME =
      "UPDATE " + "frame " + "SET " + "ts_updated = current_timestamp," + "int_mem_max_used = ?,"
  ...
  public boolean updateFrameCheckpointState(FrameInterface frame, CheckpointState state) {
    ...
    if (state.equals(CheckpointState.COMPLETE)) {
      /*
       * Only update the checkpoint state to complete if the state is either Copying or Enabled.
       */
      ...
      result = getJdbcTemplate().update("UPDATE frame SET str_checkpoint_state=? WHERE pk_frame=?",
          state.toString(), frame.getFrameId()) == 1;
    }

    /*
     * If the checkpoint state is complete or disabled then set the frame state back to waiting, if
     * and only if the frame state is currently in the checkpoint state.
     */
    if ((state.equals(CheckpointState.DISABLED))
        || state.equals(CheckpointState.COMPLETE) && result) {
      getJdbcTemplate().update("UPDATE frame SET str_state=? WHERE pk_frame=? AND str_state=?",
          FrameState.WAITING.toString(), frame.getFrameId(), FrameState.CHECKPOINT.toString());
    }
  ...
  private static final String CREATE_FRAME_STATE_OVERRIDE =
      "INSERT INTO frame_state_display_overrides (" + "pk_frame_override," + "pk_frame,"
          + "str_frame_state," + "str_override_text," + "str_rgb" + ") " + "VALUES (?,?,?,?,?)";
  ...
  private static final RowMapper<FrameStateDisplayOverride> OVERRIDE_MAPPER =
      new RowMapper<FrameStateDisplayOverride>() {
        public FrameStateDisplayOverride mapRow(ResultSet rs, int rowNum) throws SQLException {
  ...
  @Override
  public void updateFrameStateDisplayOverride(String frameId, FrameStateDisplayOverride override) {
