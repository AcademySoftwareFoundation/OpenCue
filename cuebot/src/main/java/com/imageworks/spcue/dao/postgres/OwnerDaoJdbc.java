...
public class OwnerDaoJdbc extends JdbcDaoSupport implements OwnerDao {
  ...
  public static final RowMapper<OwnerEntity> OWNER_MAPPER = new RowMapper<OwnerEntity>() {
  ...
  public OwnerEntity findOwner(String name) {
    try {
      return getJdbcTemplate().queryForObject(QUERY_FOR_OWNER + " WHERE str_username = ?",
          OWNER_MAPPER, name);
    } catch (EmptyResultDataAccessException e) {
      throw new EmptyResultDataAccessException("Failed to find owner: " + name, 1);
    }
  }

  @Override
  public OwnerEntity getOwner(String id) {
    return getJdbcTemplate().queryForObject(QUERY_FOR_OWNER + " WHERE pk_owner = ?", OWNER_MAPPER,
        id);
  }

  @Override
  public OwnerEntity getOwner(HostInterface host) {
    return getJdbcTemplate().queryForObject(QUERY_FOR_OWNER + "WHERE " + "pk_owner = (" + "SELECT "
        + "pk_owner " + "FROM " + "deed " + "WHERE " + "pk_host = ?)", OWNER_MAPPER,
        host.getHostId());
  }
  ...
  public void insertOwner(OwnerEntity owner, ShowInterface show) {
    ...
    getJdbcTemplate().update(INSERT_OWNER, owner.id, show.getShowId(), owner.name);
  }

  @Override
  public void updateShow(Entity owner, ShowInterface show) {
    getJdbcTemplate().update("UPDATE owner SET pk_show = ? WHERE pk_owner = ?", show.getShowId(),
        owner.getId());
  }
}
