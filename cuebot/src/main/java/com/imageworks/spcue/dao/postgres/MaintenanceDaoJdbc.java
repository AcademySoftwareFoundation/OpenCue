...
public class MaintenanceDaoJdbc extends JdbcDaoSupport implements MaintenanceDao {
  ...
  private static final String UPDATE_HOSTS_DOWN =
      "UPDATE " + "host_stat " + "SET " + "str_state = ? " + "WHERE " + "str_state = 'UP' " + "AND "
  ...
  public static final String LOCK_TASK =
      "UPDATE " + "task_lock " + "SET " + "int_lock = ?, " + "ts_lastrun = current_timestamp "
          + "WHERE " + "str_name = ? " + "AND " + "(int_lock = ? OR ? - int_lock > int_timeout)";
  ...
  public static final String LOCK_TASK_MIN = "UPDATE " + "task_lock " + "SET " + "int_lock = ?, "
      + "ts_lastrun = current_timestamp " + "WHERE " + "str_name= ? " + "AND " + "int_lock = ? "
      + "AND " + "interval_to_seconds(current_timestamp - ts_lastrun) > ? ";
  ...
  public void unlockTask(MaintenanceTask task) {
    getJdbcTemplate().update("UPDATE task_lock SET int_lock = 0 WHERE str_name=?", task.toString());
  }
}
