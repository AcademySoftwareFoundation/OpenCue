...
public class ShowDaoJdbc extends JdbcDaoSupport implements ShowDao {
  ...
  private static final String GET_SHOW =
      "SELECT " + "show.pk_show, " + "show.int_default_max_cores, " + "show.int_default_min_cores, "
  ...
  private static final String GET_SHOW_BY_ALIAS =
      "SELECT " + "show.pk_show, " + "show.int_default_max_cores, " + "show.int_default_min_cores, "
          + "show.int_default_max_gpus, " + "show.int_default_min_gpus, " + "show_alias.str_name, "
          + "show.b_active, " + "show.str_comment_email " + "FROM " + "show, " + "show_alias "
          + "WHERE " + "show.pk_show = show_alias.pk_show ";
  ...
  private static final String GET_PREFERRED_SHOW =
      "SELECT " + "show.pk_show, " + "show.int_default_max_cores, " + "show.int_default_min_cores, "
          + "show.int_default_max_gpus, " + "show.int_default_min_gpus, " + "show.str_name, "
          + "show.b_active, " + "show.str_comment_email " + "FROM " + "show, " + "owner," + "deed "
          + "WHERE " + "show.pk_show = owner.pk_show " + "AND " + "deed.pk_owner = owner.pk_owner "
          + "AND " + "deed.pk_host = ?";
  ...
  public void updateActive(ShowInterface s, boolean enabled) {
    getJdbcTemplate().update("UPDATE show SET b_active= ? WHERE pk_show=?", enabled, s.getShowId());
  }
  ...
  public void updateShowsStatus() {
    ...
    if (maxShowStaleDays > 0) {
      getJdbcTemplate().update("UPDATE show SET b_active=false "
          ...
          + "GROUP BY pk_show HAVING COUNT(pk_job) > 0) pk_show) " + "  AND str_name NOT IN (?)",
  ...
  public void updateFrameCounters(ShowInterface s, int exitStatus) {
    ...
    getJdbcTemplate().update("UPDATE show_stats SET " + col + " WHERE pk_show=?", s.getShowId());
  }
}
