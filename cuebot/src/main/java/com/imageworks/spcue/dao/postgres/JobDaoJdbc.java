...
public class JobDaoJdbc extends JdbcDaoSupport implements JobDao {
    ...
    public boolean isJobComplete(JobInterface job) {
        ...
        return getJdbcTemplate().queryForObject(IS_JOB_COMPLETE, Integer.class,
                job.getJobId()) == 0;
    }
    ...
    private static final String GET_JOB_DETAIL = "SELECT " + "job.pk_job," + "job.pk_show,"
            ...
            + "job.b_loki_enabled," + "job.str_loki_url," + "job.str_name," + "job.str_shot,"
    ...
    public JobDetail findLastJob(String name) {
        ...
        return getJdbcTemplate().queryForObject(
                GET_JOB_DETAIL + " AND job.str_state = 'FINISHED' AND job.str_name LIKE ? "
                        + "ORDER BY job.ts_stopped LIMIT 1",
                JOB_DETAIL_MAPPER, name);
    }
    ...
    public JobInterface findJob(String name) {
        return getJdbcTemplate().queryForObject(GET_JOB + " WHERE job.str_visible_name=?",
                JOB_MAPPER, name);
    }
    ...
    public void updatePriority(GroupInterface g, int v) {
        getJdbcTemplate().update(
                "UPDATE job_resource SET int_priority=? WHERE "
    ...
    public void updateMinCores(GroupInterface g, int v) {
        getJdbcTemplate().update(
                "UPDATE job_resource SET int_min_cores=? WHERE "
    ...
    public void updateMaxCores(GroupInterface g, int v) {
        getJdbcTemplate().update(
                "UPDATE job_resource SET int_max_cores=? WHERE "
                        + "pk_job IN (SELECT pk_job FROM job WHERE pk_folder=?)",
                v, g.getGroupId());
    }
    ...
    public void updateMaxGpus(GroupInterface g, int v) {
        getJdbcTemplate().update(
                "UPDATE job_resource SET int_max_gpus=? WHERE "
                        + "pk_job IN (SELECT pk_job FROM job WHERE pk_folder=?)",
                v, g.getGroupId());
    }
    ...
    public void updateLogPath(JobInterface job, String path) {
        getJdbcTemplate().update("UPDATE job SET str_log_dir=? WHERE pk_job=?", path,
                job.getJobId());
    }
    ...
    public void updateMaxRSS(JobInterface job, long value) {
        getJdbcTemplate().update(
                "UPDATE job_mem SET int_max_rss=? WHERE pk_job=? AND int_max_rss < ?", value,
                job.getJobId(), value);
    }
    ...
    public void insertJob(JobDetail j, JobLogUtil jobLogUtil) {
        ...
        getJdbcTemplate().update(INSERT_JOB, j.id, j.showId, j.groupId, j.facilityId, j.deptId,
                ...
                j.os, j.uid.orElse(null), j.isPaused, j.isAutoEat, j.maxRetries, j.logLokiEnabled,
                j.logLokiURL);
    }
    ...
    public void activateJob(JobInterface job, JobState jobState) {
        ...
        List<Map<String, Object>> layers = getJdbcTemplate()
                .queryForList("SELECT pk_layer, str_state, count(1) AS c FROM frame "
    ...
    public boolean isOverMaxCores(JobInterface job) {
        return getJdbcTemplate().queryForObject(IS_JOB_OVER_MAX_CORES, Integer.class,
                job.getJobId(), 0) > 0;
    }
    ...
    public boolean isAtMaxCores(JobInterface job) {
        return getJdbcTemplate().queryForObject(IS_JOB_AT_MAX_CORES, Integer.class,
                job.getJobId()) > 0;
    }
    ...
    public boolean isAtMaxGpus(JobInterface job) {
        return getJdbcTemplate().queryForObject(IS_JOB_AT_MAX_GPUS, Integer.class,
                job.getJobId()) > 0;
    }
    ...
    public void updateParent(JobInterface job, GroupDetail dest, Inherit[] inherits) {
        ...
        if (!job.getShowId().equals(dest.getShowId())) {
            throw new EntityModificationError(
                    "error moving job, " + "cannot move jobs between shows");
        }
    ...
    public void enableAutoBooking(JobInterface job, boolean value) {
        getJdbcTemplate().update("UPDATE job SET b_auto_book=? WHERE pk_job=?", value,
                job.getJobId());
    }
    ...
    public void updateEmail(JobInterface job, String email) {
        getJdbcTemplate().update("UPDATE job SET str_email=? WHERE pk_job=?", email,
                job.getJobId());
    }
