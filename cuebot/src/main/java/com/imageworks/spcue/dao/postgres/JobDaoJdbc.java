...
public class JobDaoJdbc extends JdbcDaoSupport implements JobDao {
   ...
   */
  private static final RowMapper<JobDetail> JOB_DETAIL_MAPPER = new RowMapper<JobDetail>() {
  ...
  private static final String GET_DISPATCH_JOB = "SELECT " + "job.pk_job, " + "job.pk_facility, "
      + "job.pk_show, " + "job.str_name, " + "job.str_show, " + "job.str_state, " + "job.b_paused, "
      + "job.int_max_retries, " + "job.b_autoeat, " + "job.b_auto_book," + "job.b_auto_unbook "
      + "FROM " + "job " + "WHERE " + "pk_job = ?";
  ...
  public boolean isJobComplete(JobInterface job) {
    ...
    return getJdbcTemplate().queryForObject(IS_JOB_COMPLETE, Integer.class, job.getJobId()) == 0;
  }

  public static final String GET_JOB = "SELECT " + "job.pk_job, " + "job.pk_show, " + "job.pk_dept,"
      + "job.pk_facility," + "job.str_name " + "FROM " + "job ";
  ...
  private static final String GET_JOB_DETAIL = "SELECT " + "job.pk_job," + "job.pk_show,"
      ...
      + "job.str_name," + "job.str_shot," + "job.str_state," + "job.int_uid," + "job.str_user,"
      + "job.str_email," + "job.int_frame_count," + "job.int_layer_count," + "job.ts_started,"
      + "job.ts_stopped," + "job.b_paused," + "job.int_max_retries," + "job_resource.int_max_cores,"
      + "job_resource.int_min_cores," + "job_resource.int_max_gpus," + "job_resource.int_min_gpus,"
      + "job_resource.int_priority," + "show.str_name AS show_name, "
      + "dept.str_name AS dept_name, " + "facility.str_name AS facility_name " + "FROM " + "job, "
      + "job_resource, " + "show, " + "dept, " + "facility " + "WHERE "
  ...
  private static final String FIND_JOB_BY_NAME = GET_JOB_DETAIL + "AND job.str_visible_name=? ";

  @Override
  public JobDetail getJobDetail(String id) {
    return getJdbcTemplate().queryForObject(GET_JOB_BY_ID, JOB_DETAIL_MAPPER, id);
  }
  ...
  public JobDetail findLastJob(String name) {
    ...
    return getJdbcTemplate()
        .queryForObject(GET_JOB_DETAIL + " AND job.str_state = 'FINISHED' AND job.str_name LIKE ? "
  ...
  public List<JobInterface> getJobs(TaskEntity t) {
    return getJdbcTemplate().query(GET_JOBS_BY_TASK, JOB_MAPPER, JobState.PENDING.toString(),
        t.deptId, t.shot);
  }

  @Override
  public JobDetail findJobDetail(String name) {
    return getJdbcTemplate().queryForObject(FIND_JOB_BY_NAME, JOB_DETAIL_MAPPER, name);
  }

  @Override
  public JobInterface findJob(String name) {
    return getJdbcTemplate().queryForObject(GET_JOB + " WHERE job.str_visible_name=?", JOB_MAPPER,
        name);
  }

  @Override
  public List<JobDetail> findJobs(ShowInterface show) {
    return getJdbcTemplate().query(GET_JOB_DETAIL + " AND job.pk_show=?", JOB_DETAIL_MAPPER,
        show.getShowId());
  }

  @Override
  public List<JobDetail> findJobs(GroupInterface group) {
    return getJdbcTemplate().query(GET_JOB_DETAIL + " AND job.pk_folder=?", JOB_DETAIL_MAPPER,
        group.getId());
  }
  ...
  public void updatePriority(JobInterface j, int v) {
    getJdbcTemplate().update("UPDATE job_resource SET int_priority=? WHERE pk_job=?", v,
        j.getJobId());
  }
  ...
  public void updatePriority(GroupInterface g, int v) {
    getJdbcTemplate().update("UPDATE job_resource SET int_priority=? WHERE "
        + "pk_job IN (SELECT pk_job FROM job WHERE job.pk_folder=?)", v, g.getGroupId());
  }

  @Override
  public void updateMinCores(GroupInterface g, int v) {
    getJdbcTemplate().update("UPDATE job_resource SET int_min_cores=? WHERE "
        + "pk_job IN (SELECT pk_job FROM job WHERE pk_folder=?)", v, g.getGroupId());
  }
  ...
  public void updateMinCores(JobInterface j, int v) {
    getJdbcTemplate().update("UPDATE job_resource SET int_min_cores=? WHERE pk_job=?", v,
        j.getJobId());
  }
  ...
  public void updateMinGpus(JobInterface j, int v) {
    getJdbcTemplate().update("UPDATE job_resource SET int_min_gpus=? WHERE pk_job=?", v,
        j.getJobId());
  }
  ...
  public void updatePaused(JobInterface j, boolean b) {
    getJdbcTemplate().update("UPDATE job SET b_paused=? WHERE pk_job=?", b, j.getJobId());
  }
  ...
  public void updateState(JobInterface job, JobState state) {
    getJdbcTemplate().update("UPDATE job SET str_state=? WHERE pk_job=?", state.toString(),
        job.getJobId());
  }

  @Override
  public void updateLogPath(JobInterface job, String path) {
    getJdbcTemplate().update("UPDATE job SET str_log_dir=? WHERE pk_job=?", path, job.getJobId());
  }

  @Override
  public void updateMaxRSS(JobInterface job, long value) {
    getJdbcTemplate().update("UPDATE job_mem SET int_max_rss=? WHERE pk_job=? AND int_max_rss < ?",
        value, job.getJobId(), value);
  }

  private static final String UPDATE_JOB_FINISHED = "UPDATE " + "job " + "SET " + "str_state = ?, "
      + "str_visible_name = NULL, " + "ts_stopped = current_timestamp " + "WHERE "
      + "str_state = 'PENDING' " + "AND " + "pk_job = ?";
  ...
  public boolean updateJobFinished(JobInterface job) {
    ...
    if (getJdbcTemplate().update(UPDATE_JOB_FINISHED, JobState.FINISHED.toString(),
        job.getJobId()) == 1) {
      return true;
    }
    return false;
  ...
  private static final String INSERT_JOB = "INSERT INTO " + "job " + "(" + "pk_job," + "pk_show,"
      + "pk_folder," + "pk_facility," + "pk_dept," + "str_name," + "str_visible_name," + "str_show,"
      + "str_shot," + "str_user," + "str_email," + "str_state," + "str_log_dir," + "str_os, "
      + "int_uid," + "b_paused," + "b_autoeat," + "int_max_retries " + ") "
      + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  public void insertJob(JobDetail j, JobLogUtil jobLogUtil) {
    ...
    getJdbcTemplate().update(INSERT_JOB, j.id, j.showId, j.groupId, j.facilityId, j.deptId, j.name,
        j.name, j.showName, j.shot, j.user, j.email, j.state.toString(), j.logDir, j.os,
        j.uid.orElse(null), j.isPaused, j.isAutoEat, j.maxRetries);
  }
  ...
  public boolean exists(String name) {
    try {
      return (getJdbcTemplate().queryForObject(JOB_EXISTS, Integer.class, name) >= 1);
    } catch (Exception e) {
      return false;
    }
  }
  ...
  public boolean isLaunching(JobInterface j) {
    return getJdbcTemplate().queryForObject(IS_LAUNCHING, String.class, j.getJobId())
  ...
  public void activateJob(JobInterface job, JobState jobState) {
    ...
    /*
     * Sets all frames in the setup state to Waiting. Frames with a depend count > 0 are
     * automatically updated to Depend via the update_frame_wait_to_dep trigger.
     */
    ...
    List<Map<String, Object>> layers =
        getJdbcTemplate().queryForList("SELECT pk_layer, str_state, count(1) AS c FROM frame "
    ...
    getJdbcTemplate().update(
        "UPDATE job_stat SET int_depend_count=?,int_waiting_count=? WHERE pk_job=?", jobTotals[0],
        jobTotals[1], job.getJobId());

    getJdbcTemplate().update("UPDATE job SET int_frame_count=?, int_layer_count=? WHERE pk_job=?",
  ...
  public boolean hasPendingFrames(JobInterface job) {
    try {
      return getJdbcTemplate().queryForObject(HAS_PENDING_FRAMES, Integer.class,
          job.getJobId()) > 0;
  ...
  private static final String IS_JOB_OVER_MIN_CORES =
      "SELECT " + "COUNT(1) " + "FROM " + "job_resource " + "WHERE " + "job_resource.pk_job = ? "
          + "AND " + "job_resource.int_cores > job_resource.int_min_cores";

  @Override
  public boolean isOverMinCores(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_JOB_OVER_MIN_CORES, Integer.class,
        job.getJobId()) > 0;
  }
  ...
  public boolean isOverMaxCores(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_JOB_OVER_MAX_CORES, Integer.class, job.getJobId(),
        0) > 0;
  }
  ...
  public boolean isAtMaxCores(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_JOB_AT_MAX_CORES, Integer.class, job.getJobId()) > 0;
  }
  ...
  public boolean isOverMaxGpus(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_JOB_OVER_MAX_GPUS, Integer.class, job.getJobId(),
        0) > 0;
  }
  ...
  public boolean isAtMaxGpus(JobInterface job) {
    return getJdbcTemplate().queryForObject(IS_JOB_AT_MAX_GPUS, Integer.class, job.getJobId()) > 0;
  }
  ...
  private static final String GET_FRAME_STATE_TOTALS =
      "SELECT " + "job.int_frame_count," + "job_stat.* " + "FROM " + "job," + "job_stat " + "WHERE "
          + "job.pk_job = job_stat.pk_job " + "AND " + "job.pk_job=?";
  ...
  private static final String GET_EXECUTION_SUMMARY = "SELECT " + "job_usage.int_core_time_success,"
      + "job_usage.int_core_time_fail," + "job_usage.int_gpu_time_success,"
      + "job_usage.int_gpu_time_fail," + "job_mem.int_max_rss " + "FROM " + "job," + "job_usage, "
      + "job_mem " + "WHERE " + "job.pk_job = job_usage.pk_job " + "AND "
  ...
  public void insertEnvironment(JobInterface job, Map<String, String> env) {
    for (Map.Entry<String, String> e : env.entrySet()) {
      ...
      getJdbcTemplate().update(INSERT_JOB_ENV, pk, job.getJobId(), e.getKey(), e.getValue());
    }
  }
  ...
  public void insertEnvironment(JobInterface job, String key, String value) {
    String pk = SqlUtil.genKeyRandom();
    getJdbcTemplate().update(INSERT_JOB_ENV, pk, job.getJobId(), key, value);
  }
  ...
  public Map<String, String> getEnvironment(JobInterface job) {
    ...
    List<Map<String, Object>> _result = getJdbcTemplate()
        .queryForList("SELECT str_key, str_value FROM job_env WHERE pk_job=?", job.getJobId());
  ...
  public void updateParent(JobInterface job, GroupDetail dest, Inherit[] inherits) {
    ...
    if (!job.getShowId().equals(dest.getShowId())) {
      throw new EntityModificationError("error moving job, " + "cannot move jobs between shows");
    }
    ...
    getJdbcTemplate().update("UPDATE job SET pk_folder=?, pk_dept=? WHERE pk_job=?",
        dest.getGroupId(), dest.getDepartmentId(), job.getJobId());

    getJdbcTemplate().update("UPDATE job_history SET pk_dept=? WHERE pk_job=?",
  ...
  private static final String HAS_PENDING_JOBS =
      "SELECT " + "job.pk_job " + "FROM " + "job, " + "job_stat, " + "job_resource " + "WHERE "
          + "job.pk_job = job_stat.pk_job " + "AND " + "job.pk_job = job_resource.pk_job " + "AND "
          + "job.str_state = 'PENDING' " + "AND " + "job.b_paused = false " + "AND "
          + "job.b_auto_book = true " + "AND " + "job_stat.int_waiting_count != 0 " + "AND "
  ...
  public boolean cueHasPendingJobs(FacilityInterface f) {
    return getJdbcTemplate().queryForList(HAS_PENDING_JOBS, f.getFacilityId()).size() > 0;
  }

  @Override
  public void enableAutoBooking(JobInterface job, boolean value) {
    getJdbcTemplate().update("UPDATE job SET b_auto_book=? WHERE pk_job=?", value, job.getJobId());
  }

  @Override
  public void enableAutoUnBooking(JobInterface job, boolean value) {
    getJdbcTemplate().update("UPDATE job SET b_auto_unbook=? WHERE pk_job=?", value,
        job.getJobId());
  }
  ...
  public void mapPostJob(BuildableJob job) {
    getJdbcTemplate().update(MAP_POST_JOB, SqlUtil.genKeyRandom(), job.detail.id,
  ...
  public void activatePostJob(JobInterface job) {
    getJdbcTemplate().update(ACTIVATE_POST_JOB, JobState.PENDING.toString(), job.getJobId());
  ...
  public void updateDepartment(GroupInterface group, DepartmentInterface dept) {
    getJdbcTemplate().update("UPDATE job SET pk_dept=? WHERE pk_folder=?", dept.getDepartmentId(),
        group.getGroupId());
  }

  @Override
  public void updateDepartment(JobInterface job, DepartmentInterface dept) {
    getJdbcTemplate().update("UPDATE job SET pk_dept=? WHERE pk_job=?", dept.getDepartmentId(),
        job.getJobId());
  }
  ...
  public void updateUsage(JobInterface job, ResourceUsage usage, int exitStatus) {
    ...
    if (exitStatus == 0) {
      ...
      getJdbcTemplate().update(
          "UPDATE " + "job_usage " + "SET " + "int_core_time_success = int_core_time_success + ?,"
              + "int_gpu_time_success = int_gpu_time_success + ?,"
              + "int_clock_time_success = int_clock_time_success + ?,"
              + "int_frame_success_count = int_frame_success_count + 1 " + "WHERE " + "pk_job = ? ",
          usage.getCoreTimeSeconds(), usage.getGpuTimeSeconds(), usage.getClockTimeSeconds(),
          job.getJobId());

      getJdbcTemplate().update(
          "UPDATE " + "job_usage " + "SET " + "int_clock_time_high = ? " + "WHERE " + "pk_job = ? "
              + "AND " + "int_clock_time_high < ?",
          usage.getClockTimeSeconds(), job.getJobId(), usage.getClockTimeSeconds());
    } else {

      getJdbcTemplate().update(
          "UPDATE " + "job_usage " + "SET " + "int_core_time_fail = int_core_time_fail + ?,"
  ...
  public void updateEmail(JobInterface job, String email) {
    getJdbcTemplate().update("UPDATE job SET str_email=? WHERE pk_job=?", email, job.getJobId());
  }

  public String getEmail(JobInterface job) {
    String jobId = job.getJobId();
    return getJdbcTemplate().queryForObject("SELECT str_email FROM job WHERE pk_job = ?",
        String.class, jobId);
  }
}
