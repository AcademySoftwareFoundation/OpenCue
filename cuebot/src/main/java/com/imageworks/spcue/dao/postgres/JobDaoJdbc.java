    ...
    private static final RowMapper<JobDetail> JOB_DETAIL_MAPPER =
        new RowMapper<JobDetail>() {
            public JobDetail mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                job.maxGpuUnits = rs.getInt("int_max_gpu");
                job.minGpuUnits = rs.getInt("int_min_gpu");
                job.name = rs.getString("str_name");
    ...
    public void updateParent(JobInterface job, GroupDetail dest, Inherit[] inherits) {
        ...
        for (Inherit i: inheritSet) {
            switch(i) {
                ...
                case MinGpu:
                    if (dest.jobMinGpu != CueUtil.FEATURE_DISABLED) {
                        query.append("int_min_gpu=?,");
                        values.add(dest.jobMinGpu);
                    }
                    break;

                case MaxGpu:
                    if (dest.jobMaxGpu != CueUtil.FEATURE_DISABLED) {
                        query.append("int_max_gpu=?,");
                        values.add(dest.jobMaxGpu);
                    }
                    break;
