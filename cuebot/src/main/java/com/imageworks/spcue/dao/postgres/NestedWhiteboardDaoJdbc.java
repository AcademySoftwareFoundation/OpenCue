...
public class NestedWhiteboardDaoJdbc extends JdbcDaoSupport implements NestedWhiteboardDao {
  ...
  public static final String GET_NESTED_GROUPS = "SELECT " + "show.pk_show, "
      ...
      + "folder.str_name AS group_name, " + "folder.int_job_priority as int_def_job_priority, "
      ...
      + "job.pk_job, " + "job.str_name, " + "job.str_shot, " + "job.str_user, " + "job.str_state, "
      + "job.str_log_dir, " + "job.int_uid, " + "job_resource.int_priority, " + "job.ts_started, "
      + "job.ts_stopped, " + "job.ts_updated, " + "job.b_paused, " + "job.b_autoeat, "
      + "job.b_comment, " + "COALESCE(str_os, '') AS str_os, " + "job.int_frame_count, "
      + "job.int_layer_count, " + "job_stat.int_waiting_count, " + "job_stat.int_running_count, "
      + "job_stat.int_dead_count, " + "job_stat.int_eaten_count," + "job_stat.int_depend_count, "
      ...
      + "job_resource.int_max_cores, " + "job_resource.int_max_gpus, " + "job_mem.int_max_rss "
      + "FROM " + "show, " + "dept, " + "folder_level, " + "folder_resource, " + "folder "
      + "LEFT JOIN " + "job " + "ON "
      ...
      + "job_stat " + "ON " + "(job.pk_job = job_stat.pk_job) " + "LEFT JOIN " + "job_resource "
      + "ON " + "(job.pk_job = job_resource.pk_job) " + "LEFT JOIN " + "job_usage " + "ON "
      + "(job.pk_job = job_usage.pk_job) " + "LEFT JOIN " + "job_mem " + "ON "
      + "(job.pk_job = job_mem.pk_job) " + "WHERE " + "show.pk_show = folder.pk_show " + "AND "
      + "folder.pk_folder = folder_level.pk_folder " + "AND "
      + "folder.pk_folder = folder_resource.pk_folder " + "AND " + "folder.pk_dept = dept.pk_dept ";
  ...
  class NestedJobWhiteboardMapper implements RowMapper<NestedGroup> {
    ...
    public NestedGroup mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      if (!groups.containsKey(groupId)) {
        group = NestedGroup.newBuilder().setId(rs.getString("pk_folder"))
            ...
            .setMaxGpus(rs.getInt("folder_max_gpus")).setMinGpus(rs.getInt("folder_min_gpus"))
            .setLevel(rs.getInt("int_level")).setDepartment(rs.getString("dept_name")).build();
        ...
        if (parentGroupId != null) {
          ...
          if (childrenEntry == null) {
            childrenEntry =
                new ChildrenEntry(parentGroupId, group.getLevel() - 1, rs.getString("group_name"));
      ...
      if (rs.getString("pk_job") != null) {
        ...
        group = group.toBuilder().setStats(groupStats).addJobs(rs.getString("pk_job")).build();
        groups.put(groupId, group);
      }
      return group;
    }
  ...
  private NestedJobWhiteboardMapper updateConnections(NestedJobWhiteboardMapper mapper) {
    ...
    for (ChildrenEntry entry : orderedChildren) {
      ...
      group = group.toBuilder().setGroups(childrenBuilder.build()).build();
  ...
  public NestedGroup getJobWhiteboard(ShowInterface show) {
    ...
    getJdbcTemplate().query(
        GET_NESTED_GROUPS + " AND show.pk_show=? ORDER BY folder_level.int_level ASC", mapper,
        show.getShowId());
  ...
  private static final NestedJob mapResultSetToJob(ResultSet rs) throws SQLException {
    ...
    NestedJob.Builder jobBuilder = NestedJob.newBuilder().setId(rs.getString("pk_job"))
        ...
        .setMaxGpus(rs.getInt("int_max_cores")).setMinGpus(rs.getInt("int_min_cores"))
        .setName(rs.getString("str_name")).setPriority(rs.getInt("int_priority"))
        .setShot(rs.getString("str_shot")).setShow(rs.getString("str_show"))
        .setOs(rs.getString("str_os")).setFacility(rs.getString("facility_name"))
        .setGroup(rs.getString("group_name")).setState(JobState.valueOf(rs.getString("str_state")))
  ...
  private static final String GET_HOSTS = "SELECT " + "alloc.str_name AS alloc_name, "
      + "host.pk_host, " + "host.str_name AS host_name, " + "host_stat.str_state AS host_state, "
      + "host.b_nimby, " + "host_stat.ts_booted, " + "host_stat.ts_ping, " + "host.int_cores, "
      + "host.int_cores_idle, " + "host.int_gpus, " + "host.int_gpus_idle, " + "host.int_gpu_mem, "
      + "host.int_gpu_mem_idle, " + "host.int_mem, " + "host.int_mem_idle, "
      + "host.str_lock_state, " + "host.str_tags, " + "host.b_comment, " + "host.int_thread_mode, "
      + "host_stat.str_os, " + "host_stat.int_mem_total, " + "host_stat.int_mem_free, "
      + "host_stat.int_swap_total, " + "host_stat.int_swap_free, " + "host_stat.int_mcp_total, "
      + "host_stat.int_mcp_free, " + "host_stat.int_gpu_mem_total, "
      + "host_stat.int_gpu_mem_free, " + "host_stat.int_load, " + "proc.pk_proc, "
      + "proc.int_cores_reserved AS proc_cores, " + "proc.int_gpus_reserved AS proc_gpus, "
      + "proc.int_mem_reserved AS proc_memory, " + "proc.int_mem_used AS used_memory, "
      + "proc.int_mem_max_used AS max_memory, " + "proc.int_gpu_mem_reserved AS proc_gpu_memory, "
      + "proc.ts_ping, " + "proc.ts_booked, " + "proc.ts_dispatched, " + "proc.b_unbooked, "
      + "redirect.str_name AS str_redirect, " + "job.str_name AS job_name, " + "job.str_log_dir, "
      + "show.str_name AS show_name, " + "frame.str_name AS frame_name " + "FROM " + "alloc, "
      + "host_stat, " + "host " + "LEFT JOIN " + "proc " + "ON " + "(proc.pk_host = host.pk_host) "
      + "LEFT JOIN " + "frame " + "ON " + "(proc.pk_frame = frame.pk_frame) " + "LEFT JOIN "
      + "job " + "ON " + "(proc.pk_job  = job.pk_job) " + "LEFT JOIN " + "show " + "ON "
      + "(proc.pk_show = show.pk_show) " + "LEFT JOIN " + "redirect " + "ON "
      + "(proc.pk_proc = redirect.pk_proc) " + "WHERE " + "host.pk_alloc = alloc.pk_alloc " + "AND "
      + "host.pk_host = host_stat.pk_host ";

  /**
   * Caches a the host whiteboard. This class is not thread safe so you have to synchronize calls to
  ...
  /**
   * The CachedHostWhiteboard holds onto the result of the last host whiteboard query for about 10
   * seconds, returning the same result to all subsequent requests.
   */
  private final CachedHostWhiteboard cachedHostWhiteboard = new CachedHostWhiteboard();
  ...
  public NestedHostSeq getHostWhiteboard() {
    ...
    /*
     * Ensures only 1 thread is doing the query, other threads will wait and then return the result
     * of the thead that actually did the query.
     */
    synchronized (cachedHostWhiteboard) {
      ...
      getJdbcTemplate().query(GET_HOSTS, new RowMapper<NestedHost>() {
        ...
        public NestedHost mapRow(ResultSet rs, int row) throws SQLException {
          ...
          if (pid != null) {
            ...
            if (!procs.containsKey(pid)) {
              proc = NestedProc.newBuilder().setId(pid)
                  .setName(CueUtil.buildProcName(host.getName(), rs.getInt("proc_cores"),
                      rs.getInt("proc_gpus")))
                  .setReservedCores(Convert.coreUnitsToCores(rs.getInt("proc_cores")))
                  ...
                  .setUsedMemory(rs.getLong("used_memory")).setFrameName(rs.getString("frame_name"))
                  .setJobName(rs.getString("job_name")).setShowName(rs.getString("show_name"))
                  ...
                  .setLogPath(String.format("%s/%s.%s.rqlog", rs.getString("str_log_dir"),
                      rs.getString("job_name"), rs.getString("frame_name")))
                  .setRedirectTarget(rs.getString("str_redirect")).setParent(host).build();

              host = host.toBuilder()
                  .setProcs(host.getProcs().toBuilder().addNestedProcs(proc).build()).build();
              procs.put(pid, proc);
            } else {
              proc = procs.get(pid);
            }
