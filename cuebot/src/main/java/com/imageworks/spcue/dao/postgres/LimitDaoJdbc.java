...
public class LimitDaoJdbc extends JdbcDaoSupport implements LimitDao {
  ...
  public String createLimit(String name, int maxValue) {
    ...
    getJdbcTemplate().update("INSERT INTO " + "limit_record "
        + "(pk_limit_record,str_name, int_max_value) " + "VALUES " + "(?,?,?)", limitId, name,
        maxValue);
    return limitId;
  }
  ...
  public LimitEntity findLimit(String name) {
    String findLimitQuery = GET_LIMIT_BASE + "WHERE " + "limit_record.str_name=? " + "GROUP BY "
  ...
  public LimitEntity getLimit(String id) {
    String getLimitQuery = GET_LIMIT_BASE + "WHERE " + "limit_record.pk_limit_record=? "
  ...
  public void setLimitName(LimitInterface limit, String name) {
    getJdbcTemplate().update(
        "UPDATE " + "limit_record " + "SET " + "str_name = ? " + "WHERE " + "pk_limit_record = ?",
        name, limit.getId());
  }

  public void setMaxValue(LimitInterface limit, int maxValue) {
    getJdbcTemplate().update("UPDATE " + "limit_record " + "SET " + "int_max_value = ? " + "WHERE "
        + "pk_limit_record = ?", maxValue, limit.getId());
  }
