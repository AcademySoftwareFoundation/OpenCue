...
public class LayerDaoJdbc extends JdbcDaoSupport implements LayerDao {
  ...
  public void insertLayerOutput(LayerInterface layer, String filespec) {
    getJdbcTemplate().update(INSERT_OUTPUT_PATH, UUID.randomUUID().toString(), layer.getLayerId(),
        layer.getJobId(), filespec);
  }
  ...
  public boolean isLayerDispatchable(LayerInterface l) {
    return getJdbcTemplate().queryForObject(IS_LAYER_DISPATCHABLE, Integer.class,
        l.getLayerId()) > 0;
  }

  private static final String IS_LAYER_COMPLETE =
      "SELECT " + "SUM ( " + "int_waiting_count + " + "int_running_count + " + "int_dead_count + "
          + "int_depend_count " + ") " + "FROM " + "layer_stat " + "WHERE " + "pk_layer=?";

  public boolean isLayerComplete(LayerInterface l) {
    if (isLaunching(l)) {
      return false;
    }
    return getJdbcTemplate().queryForObject(IS_LAYER_COMPLETE, Integer.class, l.getLayerId()) == 0;
  }
  ...
  public boolean isLaunching(LayerInterface l) {
    return getJdbcTemplate().queryForObject(IS_LAUNCHING, String.class, l.getJobId())
  ...
  private static final String GET_LAYER = "SELECT " + "layer.pk_layer," + "layer.pk_job,"
      + "job.pk_show," + "job.pk_facility, " + "layer.str_name " + "FROM " + "layer," + "job,"
      + "show " + "WHERE " + "layer.pk_job = job.pk_job " + "AND " + "job.pk_show = show.pk_show ";
  ...
  public LayerDetail getLayerDetail(String id) {
    LayerDetail layerDetail = getJdbcTemplate()
        .queryForObject(GET_LAYER_DETAIL + " AND layer.pk_layer=?", LAYER_DETAIL_MAPPER, id);
  ...
  public LayerDetail getLayerDetail(LayerInterface layer) {
    LayerDetail layerDetail = getJdbcTemplate().queryForObject(
        GET_LAYER_DETAIL + " AND layer.pk_layer=?", LAYER_DETAIL_MAPPER, layer.getLayerId());
    layerDetail.limits.addAll(getLimitNames(layerDetail));
    return layerDetail;
  }
  ...
  public LayerDetail findLayerDetail(JobInterface job, String name) {
    LayerDetail layerDetail = getJdbcTemplate().queryForObject(
        GET_LAYER_DETAIL + " AND layer.pk_job=? AND layer.str_name=?", LAYER_DETAIL_MAPPER,
        job.getJobId(), name);
    layerDetail.limits.addAll(getLimitNames(layerDetail));
    return layerDetail;
  }
  ...
  public LayerInterface findLayer(JobInterface job, String name) {
    try {
      return getJdbcTemplate().queryForObject(
          GET_LAYER + " AND layer.pk_job=? AND layer.str_name=?", LAYER_MAPPER, job.getJobId(),
          name);
  ...
  public List<LayerDetail> getLayerDetails(JobInterface job) {
    List<LayerDetail> layers = getJdbcTemplate().query(GET_LAYER_DETAIL + " AND layer.pk_job=?",
        LAYER_DETAIL_MAPPER, job.getJobId());
    layers.stream().forEach(layerDetail -> layerDetail.limits.addAll(getLimitNames(layerDetail)));
    return layers;
  }

  @Override
  public List<LayerInterface> getLayers(JobInterface job) {
    return getJdbcTemplate().query(GET_LAYER + " AND layer.pk_job=?", LAYER_MAPPER, job.getJobId());
  }

  @Override
  public LayerInterface getLayer(String id) {
    return getJdbcTemplate().queryForObject(GET_LAYER + " AND layer.pk_layer=?", LAYER_MAPPER, id);
  }

  private static final String INSERT_LAYER = "INSERT INTO " + "layer " + "(" + "pk_layer, "
      + "pk_job, " + "str_name, " + "str_cmd, " + "str_range, " + "int_chunk_size, "
      + "int_dispatch_order, " + "str_tags, " + "str_type," + "int_cores_min, " + "int_cores_max, "
      + "b_threadable, " + "int_mem_min, " + "int_gpus_min, " + "int_gpus_max, "
      + "int_gpu_mem_min, " + "str_services, " + "int_timeout," + "int_timeout_llu " + ") "
      + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

  @Override
  public void insertLayerDetail(LayerDetail l) {
    l.id = SqlUtil.genKeyRandom();
    getJdbcTemplate().update(INSERT_LAYER, l.id, l.jobId, l.name, l.command, l.range, l.chunkSize,
        l.dispatchOrder, StringUtils.join(l.tags, " | "), l.type.toString(), l.minimumCores,
        l.maximumCores, l.isThreadable, l.minimumMemory, l.minimumGpus, l.maximumGpus,
        l.minimumGpuMemory, StringUtils.join(l.services, ","), l.timeout, l.timeout_llu);
  }
  ...
  public void updateLayerMinMemory(LayerInterface layer, long val) {
    ...
    getJdbcTemplate().update("UPDATE layer SET int_mem_min=? WHERE pk_layer=?", val,
        layer.getLayerId());
  }

  @Override
  public void updateLayerMinGpuMemory(LayerInterface layer, long kb) {
    getJdbcTemplate().update("UPDATE layer SET int_gpu_mem_min=? WHERE pk_layer=?", kb,
        layer.getLayerId());
  }

  private static final String BALANCE_MEM = "UPDATE " + "layer " + "SET " + "int_mem_min = ? "
      + "WHERE " + "pk_layer = ? " + "AND " + "int_mem_min > ? " + "AND " + "b_optimize = true";

  @Override
  public boolean balanceLayerMinMemory(LayerInterface layer, long frameMaxRss) {

    /**
     * Lowers the memory value on the frame when the maxrss is lower than the memory requirement.
     */
    long maxrss = getJdbcTemplate().queryForObject(
        "SELECT int_max_rss FROM layer_mem WHERE pk_layer=?", Long.class, layer.getLayerId());
    ...
    boolean result = getJdbcTemplate().update(BALANCE_MEM, maxrss, layer.getLayerId(), maxrss) == 1;
    if (result) {
  ...
  public void increaseLayerMinGpuMemory(LayerInterface layer, long kb) {
    getJdbcTemplate().update(
        "UPDATE layer SET int_gpu_mem_min=? WHERE pk_layer=? AND int_gpu_mem_min < ?", kb,
        layer.getLayerId(), kb);
  }
  ...
  public void updateLayerMinCores(LayerInterface layer, int val) {
    ...
    getJdbcTemplate().update("UPDATE layer SET int_cores_min=? WHERE pk_layer=?", val,
        layer.getLayerId());
  }

  @Override
  public void updateLayerMaxCores(LayerInterface layer, int val) {
    getJdbcTemplate().update("UPDATE layer SET int_cores_max=? WHERE pk_layer=?", val,
        layer.getLayerId());
  }

  @Override
  public void updateLayerMinGpus(LayerInterface layer, int val) {
    getJdbcTemplate().update("UPDATE layer SET int_gpus_min=? WHERE pk_layer=?", val,
        layer.getLayerId());
  }

  @Override
  public void updateLayerMaxGpus(LayerInterface layer, int val) {
    getJdbcTemplate().update("UPDATE layer SET int_gpus_max=? WHERE pk_layer=?", val,
        layer.getLayerId());
  }
  ...
  public void updateLayerTags(LayerInterface layer, Set<String> tags) {
    if (tags.size() == 0) {
      throw new IllegalArgumentException("Layers must have at least one tag.");
    }
    ...
    if (sb.length() == 0) {
      throw new IllegalArgumentException(
          "Invalid layer tags, cannot contain null tags or " + "tags of zero length.");
    }
    getJdbcTemplate().update("UPDATE layer SET str_tags=? WHERE pk_layer=?", sb.toString(),
        layer.getLayerId());
  }
  ...
  public void appendLayerTags(LayerInterface layer, String val) {
    ...
    getJdbcTemplate().update(
        "UPDATE layer SET str_tags = str_tags || ? " + "WHERE pk_layer=? AND str_tags NOT LIKE ?",
        appendTag, layer.getLayerId(), matchTag);
  }

  public FrameStateTotals getFrameStateTotals(LayerInterface layer) {
    return getJdbcTemplate().queryForObject("SELECT * FROM layer_stat WHERE pk_layer=?",
  ...
  public void insertLayerEnvironment(LayerInterface layer, Map<String, String> env) {
    for (Map.Entry<String, String> e : env.entrySet()) {
      ...
      getJdbcTemplate().update(INSERT_LAYER_ENV, pk, layer.getLayerId(), layer.getJobId(),
          e.getKey(), e.getValue());
    }
  }
  ...
  public void insertLayerEnvironment(LayerInterface layer, String key, String value) {
    String pk = SqlUtil.genKeyRandom();
    getJdbcTemplate().update(INSERT_LAYER_ENV, pk, layer.getLayerId(), layer.getJobId(), key,
        value);
  }
  ...
  private static final String FIND_PAST_MAX_RSS = "SELECT " + "layer_mem.int_max_rss " + "FROM "
      + "layer, " + "layer_mem, " + "layer_stat " + "WHERE "
      + "layer.pk_layer = layer_stat.pk_layer " + "AND " + "layer.pk_layer = layer_mem.pk_layer "
      + "AND " + "layer.pk_job = ? " + "AND " + "layer.str_name = ? " + "AND "
  ...
  public long findPastMaxRSS(JobInterface job, String name) {
    try {
      long maxRss =
          getJdbcTemplate().queryForObject(FIND_PAST_MAX_RSS, Long.class, job.getJobId(), name);
      if (maxRss >= MEM_RESERVED_MIN) {
        return maxRss;
      } else {
  ...
  public void updateMinMemory(JobInterface job, long mem, LayerType type) {
    ...
    getJdbcTemplate().update("UPDATE layer SET int_mem_min=? WHERE pk_job=? AND str_type=?", mem,
        job.getJobId(), type.toString());
  }

  @Override
  public void updateMinGpuMemory(JobInterface job, long kb, LayerType type) {
    getJdbcTemplate().update("UPDATE layer SET int_gpu_mem_min=? WHERE pk_job=? AND str_type=?", kb,
        job.getJobId(), type.toString());
  }

  @Override
  public void updateMinCores(JobInterface job, int cores, LayerType type) {
    getJdbcTemplate().update("UPDATE layer SET int_cores_min=? WHERE pk_job=? AND str_type=?",
        cores, job.getJobId(), type.toString());
  }

  @Override
  public void updateMaxCores(JobInterface job, int cores, LayerType type) {
    getJdbcTemplate().update("UPDATE layer SET int_cores_max=? WHERE pk_job=? AND str_type=?",
        cores, job.getJobId(), type.toString());
  }

  @Override
  public void updateMinGpus(JobInterface job, int gpus, LayerType type) {
    getJdbcTemplate().update("UPDATE layer SET int_gpus_min=? WHERE pk_job=? AND str_type=?", gpus,
        job.getJobId(), type.toString());
  }
  ...
  public void updateTimeout(LayerInterface layer, int timeout) {
    getJdbcTemplate().update("UPDATE layer SET int_timeout=? WHERE pk_layer=?", timeout,
        layer.getLayerId());
  }
  ...
  private static final String IS_OPTIMIZABLE = "SELECT " + "COUNT(1) " + "FROM " + "layer, "
      ...
      + "AND " + "layer.pk_layer = layer_usage.pk_layer " + "AND " + "layer.pk_layer = ? " + "AND "
      + "layer.int_cores_min = 100 " + "AND " + "layer.int_gpus_min = 0 " + "AND "
  ...
  public boolean isOptimizable(LayerInterface l, int succeeded, float avg) {
    if (succeeded < 1) {
      throw new IllegalArgumentException("Succeeded frames option " + "must be greater than zero");
    }
    return getJdbcTemplate().queryForObject(IS_OPTIMIZABLE, Integer.class, l.getLayerId(),
        succeeded, avg) > 0;
  }
  ...
  private static final String THREAD_STATS =
      "SELECT " + "avg(interval_to_seconds(ts_stopped - ts_started)) AS avg, " + "int_cores, "
          ...
          + "frame.int_checkpoint_count = 0 " + "AND " + "int_cores > 0 " + "AND " + "int_gpus > 0 "
          + "GROUP BY " + "int_cores, " + "int_gpus " + "ORDER BY " + "int_cores DESC ";

  @Override
  public List<ThreadStats> getThreadStats(LayerInterface layer) {

    return getJdbcTemplate().query(THREAD_STATS, new RowMapper<ThreadStats>() {
  ...
  public void updateUsage(LayerInterface layer, ResourceUsage usage, int exitStatus) {
    ...
    if (exitStatus == 0) {
      ...
      getJdbcTemplate().update(
          "UPDATE " + "layer_usage " + "SET " + "int_core_time_success = int_core_time_success + ?,"
      ...
      getJdbcTemplate().update(
          "UPDATE " + "layer_usage " + "SET " + "int_clock_time_low = ? " + "WHERE "
              + "pk_layer = ? " + "AND " + "(? < int_clock_time_low OR int_clock_time_low = 0)",
          usage.getClockTimeSeconds(), layer.getLayerId(), usage.getClockTimeSeconds());
    } else {
      getJdbcTemplate().update(
          "UPDATE " + "layer_usage " + "SET " + "int_core_time_fail = int_core_time_fail + ?,"
              + "int_clock_time_fail = int_clock_time_fail + ?,"
              + "int_frame_fail_count = int_frame_fail_count + 1 " + "WHERE " + "pk_layer = ? ",
          usage.getCoreTimeSeconds(), usage.getClockTimeSeconds(), layer.getLayerId());
    }
  }

  private static final String INSERT_LIMIT =
      "INSERT INTO " + "layer_limit (pk_layer_limit,pk_layer,pk_limit_record)" + "VALUES (?,?,?)";
  ...
  public void dropLimit(LayerInterface layer, String limitId) {
    getJdbcTemplate().update("DELETE FROM layer_limit WHERE pk_limit_record = ? AND pk_layer = ?",
        limitId, layer.getLayerId());
  }

  @Override
  public List<LimitEntity> getLimits(LayerInterface layer) {
    return getJdbcTemplate().query(GET_LIMITS, LIMIT_MAPPER, layer.getLayerId());
  }

  @Override
  public List<String> getLimitNames(LayerInterface layer) {
    return getJdbcTemplate().query(GET_LIMIT_NAMES, LIMIT_NAME_MAPPER, layer.getLayerId());
  }
}
