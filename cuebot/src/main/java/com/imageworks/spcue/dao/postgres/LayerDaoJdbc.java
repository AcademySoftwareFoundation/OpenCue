    ...
    */
    public static final RowMapper<LayerDetail> LAYER_DETAIL_MAPPER = new RowMapper<LayerDetail>() {
        public LayerDetail mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            layer.minimumGpu = rs.getInt("int_gpu_min");
     ...
     */
     ...
     public void insertLayerDetail(LayerDetail l) {
        ...
        getJdbcTemplate().update(INSERT_LAYER,
                ...
                l.minimumCores, l.minimumGpu, l.maximumCores, l.maximumGpu, l.isThreadable,
                l.minimumMemory, l.minimumGpuMemory, StringUtils.join(l.services,","));
    }
    ...
    @Override
    public void updateLayerMinGpu(LayerInterface layer, int val) {
        getJdbcTemplate().update("UPDATE layer SET int_gpu_min=? WHERE pk_layer=?",
                val, layer.getLayerId());
    }

    @Override
    public void updateLayerMaxGpu(LayerInterface layer, int val) {
        getJdbcTemplate().update("UPDATE layer SET int_gpu_max=? WHERE pk_layer=?",
                val, layer.getLayerId());
    }
    ...
    @Override
    public void updateMinGpu(JobInterface job, int gpu, LayerType type) {
        getJdbcTemplate().update(
