...
public class SubscriptionDaoJdbc extends JdbcDaoSupport implements SubscriptionDao {
  ...
  private static final String IS_SHOW_OVER_SIZE =
      "SELECT " + "COUNT(1) " + "FROM " + "subscription s " + "WHERE " + "s.pk_show = ? " + "AND "
          + "s.pk_alloc = ? " + "AND " + "s.int_cores > s.int_size ";

  public boolean isShowOverSize(ShowInterface show, AllocationInterface alloc) {
    try {
      return getJdbcTemplate().queryForObject(IS_SHOW_OVER_SIZE, Integer.class, show.getShowId(),
          alloc.getAllocationId()) > 0;
  ...
  public boolean isShowOverSize(VirtualProc proc) {
    try {
      return getJdbcTemplate().queryForObject(IS_SHOW_OVER_SIZE, Integer.class, proc.getShowId(),
          proc.getAllocationId()) > 0;
    } catch (EmptyResultDataAccessException e) {
      return false;
    }
  }
  ...
  public boolean isShowAtOrOverSize(ShowInterface show, AllocationInterface alloc) {
    try {
      return getJdbcTemplate().queryForObject(IS_SHOW_AT_OR_OVER_SIZE, Integer.class,
          show.getShowId(), alloc.getAllocationId()) > 0;
    } catch (EmptyResultDataAccessException e) {
      return false;
    }
  }
  ...
  private static final String IS_SHOW_OVER_BURST =
      "SELECT " + "COUNT(1) " + "FROM " + "subscription s " + "WHERE " + "s.pk_show = ? " + "AND "
          + "s.pk_alloc = ? " + "AND " + "s.int_cores + ? > s.int_burst";

  @Override
  public boolean isShowOverBurst(ShowInterface show, AllocationInterface alloc, int coreUnits) {
    try {
      return getJdbcTemplate().queryForObject(IS_SHOW_OVER_BURST, Integer.class, show.getShowId(),
          alloc.getAllocationId(), coreUnits) > 0;
    } catch (EmptyResultDataAccessException e) {
      return true;
    }
  }
  ...
  public boolean isShowAtOrOverBurst(ShowInterface show, AllocationInterface alloc) {
    try {
      return getJdbcTemplate().queryForObject(IS_SHOW_AT_OR_OVER_BURST, Integer.class,
          show.getShowId(), alloc.getAllocationId()) > 0;
    } catch (EmptyResultDataAccessException e) {
      return true;
    }
  ...
  private static final String GET_SUB = "SELECT " + "subscription.pk_alloc,"
      ...
      + "subscription.pk_subscription," + "(alloc.str_name || '.' || show.str_name) AS str_name "
      + "FROM " + "subscription," + "alloc," + "show," + "facility " + "WHERE "
      + "subscription.pk_show = show.pk_show " + "AND " + "subscription.pk_alloc = alloc.pk_alloc "
      + "AND " + "alloc.pk_facility = facility.pk_facility ";
  ...
  private static final String INSERT_SUBSCRIPTION = "INSERT INTO " + "subscription " + "( "
      + "pk_subscription, pk_alloc, pk_show, int_size, int_burst" + ") " + "VALUES (?,?,?,?,?)";

  public void insertSubscription(SubscriptionEntity detail) {
    detail.id = SqlUtil.genKeyRandom();
    getJdbcTemplate().update(INSERT_SUBSCRIPTION, detail.id, detail.allocationId, detail.showId,
  ...
  public boolean hasRunningProcs(SubscriptionInterface sub) {
    try {
      return getJdbcTemplate().queryForObject(HAS_RUNNING_PROCS, Integer.class,
          sub.getSubscriptionId()) > 0;
  ...
  public void deleteSubscription(SubscriptionInterface sub) {
    ...
    getJdbcTemplate().update("DELETE FROM subscription WHERE pk_subscription=?",
        sub.getSubscriptionId());
  }

  public void updateSubscriptionSize(SubscriptionInterface sub, int size) {
    getJdbcTemplate().update("UPDATE subscription SET int_size=? WHERE pk_subscription=?", size,
        sub.getSubscriptionId());
  }

  public void updateSubscriptionBurst(SubscriptionInterface sub, int size) {
    getJdbcTemplate().update("UPDATE subscription SET int_burst=? WHERE pk_subscription=?", size,
        sub.getSubscriptionId());
  }
}
