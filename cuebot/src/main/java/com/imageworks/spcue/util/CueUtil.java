...
public final class CueUtil {
  ...
  /**
   * Features that relay on an integer greated than 0 to work properly are disabled by setting them
   * to -1.
   */
  ...
  /**
   * Return true if the given name is formatted as a valid allocation name. Allocation names should
   * be facility.unique_name.
   *
   * @param name
   * @return
   */
  ...
  public static List<Integer> normalizeFrameRange(FrameSet frameSet, int chunkSize) {
    ...
    if (chunkSize > 1) {
      ...
      if (chunkSize > rangeSize) {
         ...
         */
        final Set<Integer> tempResult = new LinkedHashSet<Integer>((rangeSize / chunkSize) + 1);
