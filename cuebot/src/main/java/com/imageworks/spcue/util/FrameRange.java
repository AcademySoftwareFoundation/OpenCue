...
public class FrameRange {
   ...
   *
   * A stepSize designated with y creates an inverted step. Frames that would be included with an x
   * step are excluded.
   *
  ...
  private static ImmutableList<Integer> getSteppedRange(Integer start, Integer end, Integer step,
    ...
    if (inverseStep) {
      ...
      return ImmutableList
          .copyOf(Collections2.filter(fullRange, Predicates.not(Predicates.in(steppedRange))));
    }
    return steppedRange;
  }
