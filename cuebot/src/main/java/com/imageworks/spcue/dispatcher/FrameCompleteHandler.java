...
public class FrameCompleteHandler {
   ...
   * more dispatch threads than booking threads, the dispatcher will over compensate and unbook too
   * many cores if an imbalance occurs. Its better to keep cores running the same place for cache
   * coherence.
   */
  ...
  public void handleFrameCompleteReport(final FrameCompleteReport report) {
    ...
    if (isShutdown()) {
      throw new RqdRetryReportException(
          "Error processing the frame complete report, " + "cuebot not accepting packets.");
    }
    ...
    try {
      ...
      if (dispatchSupport.stopFrame(frame, newFrameState, exitStatus,
        ...
        if (dispatcher.isTestMode()) {
          ...
          handlePostFrameCompleteOperations(proc, report, job, frame, newFrameState, frameDetail);
        } else {
        ...
        /*
         * First check if we have a redirect. When a user retries a frame the proc is redirected
         * back to the same job without checking any other properties.
         */
      ...
      /*
       * Do not propagate this exception to RQD. This usually means the cue lost connectivity to the
       * host and cleared out the record of the proc. If this is propagated back to RQD, RQD will
      ...
      throw new RqdRetryReportException(
          "error processing the frame complete " + "report, sending retry message to RQD " + e, e);
    }
  }
  ...
  public void handlePostFrameCompleteOperations(VirtualProc proc, FrameCompleteReport report,
      DispatchJob job, DispatchFrame frame, FrameState newFrameState, FrameDetail frameDetail) {
    try {
      ...
      if (newFrameState.equals(FrameState.SUCCEEDED) && !isLayerComplete) {
        /*
         * If the layer meets some specific criteria then try to update the minimum memory and tags
         * so it can run on a wider variety of cores, namely older hardware.
         */
       ...
       * Succeeded before doing an isJobComplete check, then jobs that finish with the auto-eat flag
       * enabled will not leave the cue.
       */
      ...
      /*
       * Some exit statuses indicate that a frame was killed by the application due to a memory
       * issue and should be retried. In this case, disable the optimizer and raise the memory by
       * what is specified in the show's service override, service or 2GB.
       */
      if (report.getExitStatus() == Dispatcher.EXIT_STATUS_MEMORY_FAILURE
        ...
        try {
          ...
          ServiceOverride showService =
              whiteboardDao.getServiceOverride(showDao.findShowDetail(frame.show), serviceName);
          ...
          logger.info(
              "Using service default for mem increase: " + Math.floor(increase / 1024) + "Mb.");
        }
      ...
      /*
       * An exit status of FAILED_LAUNCH (256) indicates that the frame could not be launched due to
       * some unforeseen unrecoverable error that is not checked when the launch command is given.
       * The most common cause of this is when the job log directory is removed before the job is
       * complete.
       *
      ...
      if (job.state.equals(JobState.FINISHED)
        ...
        /*
         * Only rebook whole cores that have not been locally dispatched. Rebooking fractional can
         * cause storms of booking requests that don't have a chance of finding a suitable frame to
         * run.
         */
        if (!proc.isLocalDispatch && proc.coresReserved >= 100
          ...
          bookingQueue.execute(
              new DispatchBookHost(hostManager.getDispatchHost(proc.getHostId()), dispatcher, env));
        }
      ...
      /*
       * If the job is marked unbookable and its over its minimum value, we check to see if the proc
       * can be moved to a job that hasn't reached its minimum proc yet.
       *
      ...
      if (newFrameState.equals(FrameState.WAITING) || newFrameState.equals(FrameState.SUCCEEDED)) {
      ...
      /*
       * At this point, the proc has no place to go. Since we've run into an error its best to just
       * unbook it. You can't handle this with a roll back because the record existed before any
       * transactions started.
       */
      logger.warn("An error occured when procssing " + "frame complete message, "
          + CueExceptionUtil.getStackTrace(e));
      try {
        dispatchSupport.unbookProc(proc, "an error occured when procssing frame complete message.");
  ...
  /**
   * Determines the new FrameState for a frame based on values contained in the FrameCompleteReport
   *
   * If the frame is Waiting or Eaten, then it was manually set to that status before the frame was
   * killed. In that case whatever the current state in the DB is the one we want to use.
   *
   * If the frame status is dead or the frame.exitStatus is a non-zero value, and the frame has been
   * retried job.maxRetries times, then the frame is Dead. If the frame has an exit status of 256,
   * that is a non-retry status, the frame is dead.
   *
   * Assuming the two previous checks are not true, then a non-zero exit status sets the frame back
   * to Waiting, while a zero status sets the frame to Succeeded.
   *
   * @param job
   * @param frame
   * @param report
  ...
  public static final FrameState determineFrameState(DispatchJob job, LayerDetail layer,
    ...
    // Checks for frames that have reached max retries.
      ...
      if (report.getExitStatus() == FrameExitStatus.SKIP_RETRY_VALUE
        ...
        report =
            FrameCompleteReport.newBuilder(report).setExitStatus(report.getExitStatus()).build();
        newState = FrameState.WAITING;
      } else if (job.autoEat) {
