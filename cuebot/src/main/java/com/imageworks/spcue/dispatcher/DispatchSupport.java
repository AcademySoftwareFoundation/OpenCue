...
public interface DispatchSupport {
  ...
  /**
   * Updates the frame to the Running state and Reserve the resources in the specified proc for the
   * specified frame. If the proc does not exist, its inserted, otherwise its updated.
   *
  ...
  /**
   * This method clears out a proc that was lost track of. This can happen if the host fails and the
   * proc fails to report in, a network outage occurs, or something of that nature.
   *
   * @param proc
   * @param reason
   * @param exitStatus
   ...
   *
   * Once the RunFrame object is created string replacement is done in the frame command to replace
   * any tags in the command.
   *
   * Currently these tags are supported: [IFRAME] - integer frame (no padding) [ZFRAME] - 4 padded
   * frame [LAYER] - the layer name [JOB] - the job name [IFRAME] - the full frame name [JOB] - the
   * job name [LAYER] - the layer name
   *
   * @param proc
   * @param frame
   * @return RunFrame
  ...
  /**
   * Checks to see if job passes basic tests for dispatchability. Tests include if the proc is over
   * its max, if it has pending frames, and if its paused.
   *
   * @param job
   ...
   * @param lluTime
   */
  void updateFrameMemoryUsageAndLluTime(FrameInterface frame, long rss, long maxRss, long lluTime);

  /**
   * Update memory usage data for a given frame's proc record. The frame is used to update the proc
   * so the update fails if the proc has been rebooked onto a new frame.
   *
   * @param frame
   * @param rss
   * @param maxRss
   ...
   */
  void updateProcMemoryUsage(FrameInterface frame, long rss, long maxRss, long vsize, long maxVsize,
