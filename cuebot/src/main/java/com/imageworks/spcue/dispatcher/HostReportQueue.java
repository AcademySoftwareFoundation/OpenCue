...
public class HostReportQueue extends ThreadPoolExecutor {
  ...
  public void execute(DispatchHandleHostReport newReport) {
    ...
    hostMap.put(newReport.getKey(), new HostReportWrapper(newReport.getHostReport(), newReport));
  ...
  public void shutdown() {
    if (!isShutdown.getAndSet(true)) {
      logger.info(
          "Shutting down report pool, currently " + this.getActiveCount() + " active threads.");

      final long startTime = System.currentTimeMillis();
      while (this.getQueue().size() != 0 && this.getActiveCount() != 0) {
        try {
          logger.info(
              "report pool is waiting for " + this.getQueue().size() + " more units to complete");
