...
public class HealthyThreadPool extends ThreadPoolExecutor {
  ...
  public HealthyThreadPool(String name, int healthThreshold, int minUnhealthyPeriodMin,
    ...
    this(name, healthThreshold, minUnhealthyPeriodMin, poolSize, threadsMinimum, threadsMaximum, 0);
  }
  ...
  private static long diffInMinutes(Date dateStart, Date dateEnd) {
    return TimeUnit.MINUTES.convert(dateEnd.getTime() - dateStart.getTime(), TimeUnit.MILLISECONDS);
  }
  ...
  public int sleepTime() {
    if (!isShutdown.get()) {
      int sleep = (int) (baseSleepTimeMillis
          - (((this.getQueue().size() / (float) this.poolSize) * baseSleepTimeMillis)) * 2);
      if (sleep < 0) {
        sleep = 0;
      }
      return sleep;
      ...
      while (this.getQueue().size() != 0 && this.getActiveCount() != 0) {
        try {
          if (System.currentTimeMillis() - startTime > 10000) {
            throw new InterruptedException(name + " thread pool failed to shutdown properly");
          }
          Thread.sleep(250);
        } catch (InterruptedException e) {
