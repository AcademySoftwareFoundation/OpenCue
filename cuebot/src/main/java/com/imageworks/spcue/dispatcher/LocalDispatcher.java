...
public class LocalDispatcher extends AbstractDispatcher implements Dispatcher {
    ...
    for (LocalHostAssignment lha : lhas) {
      ...
      switch (lha.getType()) {
        ...
        case LAYER_PARTITION:
          procs.addAll(dispatchHost(host, jobManager.getLayerDetail(lha.getLayerId()), lha));
          break;
  ...
  private List<VirtualProc> dispatchHost(DispatchHost host, JobInterface job,
    ...
    for (DispatchFrame frame : frames) {
       ...
       */
      if (!lha.hasAdditionalResources(lha.getThreads() * 100, frame.getMinMemory(), frame.minGpus,
          frame.minGpuMemory)) {
        continue;
      }
      ...
      /*
       * Dispatch the frame. If a frame is booked, dispatchHost returns true, else if returns false.
       * If the dispatch fails in a way that we should stop dispatching immediately (the host is
       * down), a DispatcherException is thrown.
       */
      if (dispatchHost(frame, proc)) {
        ...
        /*
         * This should stay here and not go into VirtualProc or else the count will be off if you
         * fail to book.
         */
  ...
  public List<VirtualProc> dispatchHost(DispatchHost host, JobInterface job) {
    /*
     * Load up the local assignment. If one doesn't exist, that means the user has removed it and no
     * booking action should be taken.
     */
    LocalHostAssignment lha =
  ...
  private List<VirtualProc> dispatchHost(DispatchHost host, LayerInterface layer,
    ...
    for (DispatchFrame frame : frames) {
      ...
      if (dispatchHost(frame, proc)) {
        ...
        lha.useResources(proc.coresReserved, proc.memoryReserved, proc.gpusReserved,
            proc.gpuMemoryReserved);
        if (!lha.hasAdditionalResources(100, memReservedMin, Dispatcher.GPU_UNITS_RESERVED_MIN,
            memGpuReservedMin)) {
          break;
        }
  ...
  public void dispatchProcToJob(VirtualProc proc, JobInterface job) {
    ...
    switch (lha.getType()) {
      ...
      case LAYER_PARTITION:
        frames = dispatchSupport.findNextDispatchFrames(jobManager.getLayer(proc.getLayerId()),
    ...
    logger.info("Frames found: " + frames.size() + " for host " + proc + " " + proc.coresReserved
        + "/" + proc.memoryReserved + " on job " + job.getName());
