...
public class HostReportHandler {
  ...
  Cache<String, Long> killRequestCounterCache = CacheBuilder.newBuilder()
      .expireAfterWrite(FRAME_KILL_CACHE_EXPIRE_AFTER_WRITE_MINUTES, TimeUnit.MINUTES).build();
    ...
    try {
      ...
      try {
        ...
        hostManager.setHostStatistics(host, rhost.getTotalMem(), rhost.getFreeMem(),
            rhost.getTotalSwap(), rhost.getFreeSwap(), rhost.getTotalMcp(), rhost.getFreeMcp(),
            rhost.getTotalGpuMem(), rhost.getFreeGpuMem(), rhost.getLoad(),
            new Timestamp(rhost.getBootTime() * 1000l), rhost.getAttributesMap().get("SP_OS"));
        ...
        /**
         * This should only happen at boot time or it will fight with the dispatcher over row locks.
         */
        if (isBoot) {
        ...
        logger.info("Unable to find host " + rhost.getName() + "," + dae + " , creating host.");
      ...
      /*
       * Updates memory usage for the proc, frames, jobs, and layers. And LLU time for the frames.
       */
      ...
      /*
       * The checks are done in order of least CPU intensive to most CPU intensive, saving checks
       * that hit the DB for last.
       *
       * These are done so we don't populate the booking queue with a bunch of hosts that can't be
       * booked.
       */
      String msg = null;
      ...
      if (!isTempDirStorageEnough(report.getHost().getTotalMcp(), report.getHost().getFreeMcp(),
          host.getOs())) {
        msg = String.format(
            "%s doesn't have enough free space in the temporary directory (mcp), %dMB", host.name,
            (report.getHost().getFreeMcp() / 1024));
        ...
        msg = String.format("%s doesn't have enough free system mem, %d needs %d", host.name,
            report.getHost().getFreeMem(), memReservedMin);
      ...
      if (msg != null) {
        ...
        if (hostManager.isPreferShow(host)) {
          bookingQueue.execute(
              new DispatchBookHost(host, hostManager.getPreferredShow(host), dispatcher, env));
          return;
        }
      ...
      if (reportQueue.getQueue().size() > 0 || System.currentTimeMillis() - startTime > 100) {
        ...
        CueUtil.logDuration(startTime,
            "host report " + report.getHost().getName() + " with " + report.getFramesCount()
                + " running frames, waiting: " + reportQueue.getQueue().size());
      }
    }
  }
   ...
   *
   * Use dispatcher.min_available_temp_storage_percentage (opencue.properties) to define what's the
   * accepted threshold. Providing hostOs is necessary as this feature is currently not available on
   * Windows hosts
   *
  ...
  private boolean isTempDirStorageEnough(Long tempTotalStorage, Long tempFreeStorage,
    ...
    int minAvailableTempPercentage =
        env.getRequiredProperty("dispatcher.min_available_temp_storage_percentage", Integer.class);
   ...
   *
   * If a host pings in with a different hardware state than what is currently in the DB, the state
   * is updated. If the hardware state is Rebooting or RebootWhenIdle, then state can only be
   * updated with a boot report. If the state is Repair, then state is never updated via RQD.
   *
   * @param host
   * @param reportState
   * @param isBoot
   ...
   * free space in the temporary directory is greater or equal to the minimum required and the host
   * has a comment with subject: SUBJECT_COMMENT_FULL_TEMP_DIR
   *
   * @param host
   * @param reportHost
   * @return
  ...
  private boolean changeStateForTempDirStorage(DispatchHost host, RenderHost reportHost) {
    ...
    // Prevent cue frames from booking on hosts with full temporary directories
    boolean hasEnoughTempStorage =
        isTempDirStorageEnough(reportHost.getTotalMcp(), reportHost.getFreeMcp(), host.getOs());
    if (!hasEnoughTempStorage && host.hardwareState == HardwareState.UP) {
      ...
      c.timestamp = null;
      long requiredTempMb =
          (long) (((minAvailableTempPercentage / 100.0) * reportHost.getTotalMcp()) / 1024);
      c.message =
          "Host " + host.getName() + " marked as REPAIR. The current amount of free space in the "
  ...
  /**
   * Changes the NIMBY lock state. If the DB indicates a NIMBY lock but RQD does not, then the host
   * is unlocked. If the DB indicates the host is not locked but RQD indicates it is, the host is
   * locked.
   *
   * @param host
   * @param rh
  ...
  /**
   * Changes the Lock state of the host. Looks at the number of locked cores and sets host to locked
   * if all cores are locked.
   *
   ...
   * taking more than OOM_FRAME_OVERBOARD_PERCENT of what it had reserved For frames that are using
   * more than they had reserved but not above the threshold, negotiate expanding the reservations
   * with other frames on the same host
   *
   * @param dispatchHost
   * @param report
   */
  private void handleMemoryUsage(final DispatchHost dispatchHost, RenderHost renderHost,
    ...
    final double OOM_MAX_SAFE_USED_SWAP_THRESHOLD =
        env.getRequiredProperty("dispatcher.oom_max_safe_used_swap_memory_threshold", Double.class);
    ...
    if (memoryWarning) {
      ...
      runningFrames.sort(Comparator
          .comparingLong((RunningFrameInfo frame) -> frame.getUsedSwapMemory()).reversed());
      ...
      for (RunningFrameInfo frame : runningFrames) {
        ...
        if (killProcForMemory(frame.getFrameId(), renderHost.getName(), KillCause.HostUnderOom)) {
          swapUsed -= frame.getUsedSwapMemory();
          logger.info("Memory warning(" + renderHost.getName() + "): " + "Killing frame on "
              + frame.getJobName() + "." + frame.getFrameName() + ", using too much swap.");
        }
  ...
  public enum KillCause {
    FrameOverboard("This frame is using more memory than it had reserved."), HostUnderOom(
        "Frame killed by host under OOM pressure"), FrameTimedOut(
            "Frame timed out"), FrameLluTimedOut("Frame LLU timed out"), FrameVerificationFailure(
    ...
    if (cachedCount > FRAME_KILL_RETRY_LIMIT) {
      ...
      if (!dispatcher.isTestMode()) {
        try {
          ...
          logger.info("Trying to kill a frame that no longer exists: host=" + hostname + " frameId="
              + frameId);
        }
      }
      return false;
  ...
  private boolean killProcForMemory(String frameId, String hostname, KillCause killCause) {
    ...
    if (dispatcher.isTestMode()) {
      ...
      try {
        killQueue.execute(new DispatchRqdKillFrameMemory(hostname, frame, killCause.toString(),
            rqdClient, dispatchSupport, dispatcher.isTestMode()));
  ...
  private boolean killFrame(String frameId, String hostname, KillCause killCause) {
    ...
    if (dispatcher.isTestMode()) {
      ...
      try {
        killQueue
            .execute(new DispatchRqdKillFrame(hostname, frameId, killCause.toString(), rqdClient));
        prometheusMetrics.incrementFrameKilledCounter(hostname, killCause);
      } catch (TaskRejectedException e) {
        logger.warn("Unable to add a DispatchRqdKillFrame request, task rejected, " + e);
      }
  ...
  private boolean isFrameOverboard(final RunningFrameInfo frame) {
    ...
    try {
      ...
      logger.info(
          "HostReportHandler(isFrameOverboard): Virtual proc for frame " + frame.getFrameName()
              + " on job " + frame.getJobName() + " doesn't exist on the database");
  ...
  private void handleMemoryReservations(final RunningFrameInfo frame) {
    ...
    try {
      ...
      if (proc != null) {
        ...
        logger.info("frame " + frame.getFrameName() + " on job " + frame.getJobName()
            + "was unable to reserve an additional " + CueUtil.KbToMb(memNeeded) + "on proc "
            + proc.getName() + ", " + e);
        try {
  ...
  private void updateMemoryUsageAndLluTime(List<RunningFrameInfo> rFrames) {
    for (RunningFrameInfo rf : rFrames) {
      ...
      dispatchSupport.updateProcMemoryUsage(frame, rf.getRss(), rf.getMaxRss(), rf.getVsize(),
          rf.getMaxVsize(), rf.getUsedGpuMemory(), rf.getMaxUsedGpuMemory(), rf.getUsedSwapMemory(),
  ...
  /**
   * Verify all running frames in the given report against the DB. Frames that have not been running
   * for at least FRAME_VERIFICATION_GRACE_PERIOD_SECONDS are skipped.
   *
   * If a frame->proc mapping is not verified then the record for the proc is pulled from the DB. If
   * the proc doesn't exist at all, then the frame is killed with the message: "but the DB did not
   * reflect this"
   *
  ...
  public List<RunningFrameInfo> verifyRunningFrameInfo(HostReport report) {
    ...
    for (RunningFrameInfo runningFrame : report.getFramesList()) {
      ...
      if (runtimeSeconds < FRAME_VERIFICATION_GRACE_PERIOD_SECONDS) {
        logger.info(
            "verified " + runningFrame.getJobName() + "/" + runningFrame.getFrameName() + " on "
                + report.getHost().getName() + " by grace period " + runtimeSeconds + " seconds.");
      ...
      /*
       * The frame this proc is running is no longer assigned to this proc. Don't ever touch the
       * frame record. If we make it here that means the proc has been running for over 2 min.
       */
      String msg;
      VirtualProc proc = null;
      ...
      try {
        ...
        msg = "Virtual proc " + proc.getProcId() + "is assigned to " + proc.getFrameId() + " not "
            + runningFrame.getFrameId();
      } catch (Exception e) {
        /*
         * This will happen if the host goes offline and then comes back. In this case, we don't
         * touch the frame since it might already be running somewhere else. We do however kill the
         * proc.
         */
      ...
      if (proc == null) {
        ...
        if (frameLatestVersion.state != FrameState.RUNNING) {
          logger.info("DelayedVerification, the proc " + runningFrame.getResourceId() + " on host "
              + report.getHost().getName() + " has already Completed " + runningFrame.getJobName()
          ...
          logger.info("FrameVerificationError, the proc " + runningFrame.getResourceId()
              ...
              + runningFrame.getFrameName() + " but the DB did not " + "reflect this. " + msg);
        } else {
