...
public class ManageLimit extends LimitInterfaceGrpc.LimitInterfaceImplBase {
  ...
  public void find(LimitFindRequest request, StreamObserver<LimitFindResponse> responseObserver) {
    LimitFindResponse response =
        LimitFindResponse.newBuilder().setLimit(whiteboard.findLimit(request.getName())).build();
    responseObserver.onNext(response);
    responseObserver.onCompleted();
  }
  ...
  public void get(LimitGetRequest request, StreamObserver<LimitGetResponse> responseObserver) {
    LimitGetResponse response =
        LimitGetResponse.newBuilder().setLimit(whiteboard.getLimit(request.getId())).build();
    responseObserver.onNext(response);
    responseObserver.onCompleted();
  }
  ...
  public void getAll(LimitGetAllRequest request,
    ...
    responseObserver
        .onNext(LimitGetAllResponse.newBuilder().addAllLimits(whiteboard.getLimits()).build());
    responseObserver.onCompleted();
  }
  ...
  public void rename(LimitRenameRequest request,
    ...
    adminManager.setLimitName(adminManager.findLimit(request.getOldName()), request.getNewName());
  ...
  public void setMaxValue(LimitSetMaxValueRequest request,
    ...
    adminManager.setLimitMaxValue(adminManager.findLimit(request.getName()), request.getMaxValue());
