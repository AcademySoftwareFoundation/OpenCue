...
public class ManageProc extends ProcInterfaceGrpc.ProcInterfaceImplBase {
  ...
  public void unbookProcs(ProcUnbookProcsRequest request,
    ...
    responseObserver.onNext(ProcUnbookProcsResponse.newBuilder().setNumProcs(jobManagerSupport
        .unbookProcs(procSearch, request.getKill(), new Source(request.toString()))).build());
    responseObserver.onCompleted();
  }
  ...
  public void unbookToGroup(ProcUnbookToGroupRequest request,
    ...
    if (request.getR().getMaxResultsCount() == 0) {
      throw new RuntimeException(
          "You must specify the number of procs to unbook " + "within the ProcSearchCriteria.");
    }
  ...
  public void unbookToJob(ProcUnbookToJobRequest request,
    ...
    if (jobs.size() == 0) {
      ...
      List<VirtualProc> procs = redirectManager.addRedirect(request.getR(), jobs, request.getKill(),
          new Source(request.toString()));

      returnVal = procs.size();
    }
    responseObserver.onNext(ProcUnbookToJobResponse.newBuilder().setNumProcs(returnVal).build());
    responseObserver.onCompleted();
  }
  ...
  public void getHost(ProcGetHostRequest request,
    ...
    VirtualProc proc = getVirtualProc(request.getProc());
    ProcGetHostResponse response =
        ProcGetHostResponse.newBuilder().setHost(whiteboard.getHost(proc.getHostId())).build();
    responseObserver.onNext(response);
    responseObserver.onCompleted();
  }
