...
public class ManageHost extends HostInterfaceGrpc.HostInterfaceImplBase {
  ...
  public void getHost(HostGetHostRequest request,
    ...
    try {
      responseObserver.onNext(
          HostGetHostResponse.newBuilder().setHost(whiteboard.getHost(request.getId())).build());
      responseObserver.onCompleted();
    } catch (EmptyResultDataAccessException e) {
      responseObserver.onError(
          Status.NOT_FOUND.withDescription(e.getMessage()).withCause(e).asRuntimeException());
  ...
  public void setAllocation(HostSetAllocationRequest request,
    ...
    HostInterface host = getHostInterface(request.getHost());
    hostManager.setAllocation(host, adminManager.getAllocationDetail(request.getAllocationId()));
  ...
  public void getComments(HostGetCommentsRequest request,
    ...
    responseObserver.onNext(HostGetCommentsResponse.newBuilder().setComments(commentSeq).build());
    responseObserver.onCompleted();
  }
  ...
  public void getOwner(HostGetOwnerRequest request,
    ...
    HostInterface host = getHostInterface(request.getHost());
    responseObserver
        .onNext(HostGetOwnerResponse.newBuilder().setOwner(whiteboard.getOwner(host)).build());
    responseObserver.onCompleted();
  }
  ...
  public void redirectToJob(HostRedirectToJobRequest request,
    ...
    boolean value = redirectManager.addRedirect(virtualProcs, jobManager.getJob(request.getJobId()),
