...
public class ManageFrame extends FrameInterfaceGrpc.FrameInterfaceImplBase {
  ...
  public void getFrames(FrameGetFramesRequest request,
    ...
    responseObserver.onNext(FrameGetFramesResponse.newBuilder()
        .setFrames(whiteboard.getFrames(frameSearchFactory
            .create(jobManagerSupport.getJobManager().findJob(request.getJob()), request.getR())))
        .build());
    responseObserver.onCompleted();
  }
  ...
  public void createDependencyOnFrame(FrameCreateDependencyOnFrameRequest request,
    ...
    FrameEntity frame = getFrameEntity(request.getFrame());
    FrameOnFrame depend =
        new FrameOnFrame(frame, jobManager.getFrameDetail(request.getDependOnFrame().getId()));
    dependManager.createDepend(depend);
    Depend dependency = whiteboard.getDepend(depend);
    responseObserver
        .onNext(FrameCreateDependencyOnFrameResponse.newBuilder().setDepend(dependency).build());
    responseObserver.onCompleted();
  }
  ...
  public void createDependencyOnJob(FrameCreateDependencyOnJobRequest request,
    ...
    Depend dependency = whiteboard.getDepend(depend);
    responseObserver
        .onNext(FrameCreateDependencyOnJobResponse.newBuilder().setDepend(dependency).build());
    responseObserver.onCompleted();
  }
  ...
  public void createDependencyOnLayer(FrameCreateDependencyOnLayerRequest request,
    ...
    Depend dependency = whiteboard.getDepend(depend);
    responseObserver
        .onNext(FrameCreateDependencyOnLayerResponse.newBuilder().setDepend(dependency).build());
    responseObserver.onCompleted();
  }
  ...
  public void addRenderPartition(FrameAddRenderPartitionRequest request,
    ...
    if (localBookingSupport.bookLocal(frame, request.getHost(), request.getUsername(), lha)) {
      ...
      responseObserver.onNext(
          FrameAddRenderPartitionResponse.newBuilder().setRenderPartition(partition).build());
      responseObserver.onCompleted();
    } else {
  ...
  public void getFrameStateDisplayOverrides(GetFrameStateDisplayOverridesRequest request,
    ...
    try {
      ...
      responseObserver.onError(Status.INTERNAL
          .withDescription("No Frame State display overrides found.").asRuntimeException());
    }
  }
