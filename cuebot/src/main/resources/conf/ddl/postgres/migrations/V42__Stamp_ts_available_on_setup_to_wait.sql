-- V41 added a BEFORE INSERT trigger to stamp frame.ts_available when frames
...
-- UPDATE trigger that stamps ts_available when frames first move
-- SETUP -> WAITING, and backfills any frames that already made the jump
-- before this migration.

CREATE OR REPLACE FUNCTION trigger__set_frame_ts_available_setup_to_wait()
...
CREATE TRIGGER set_frame_ts_available_setup_to_wait BEFORE UPDATE ON frame
FOR EACH ROW
  WHEN (OLD.str_state = 'SETUP' AND NEW.str_state = 'WAITING')
  EXECUTE PROCEDURE trigger__set_frame_ts_available_setup_to_wait();

-- Backfill ts_available for frames that have already left SETUP but never got
...
-- Frames still in SETUP or DEPEND legitimately keep ts_available NULL until
-- they unblock.
UPDATE frame f
