CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_depend_active_er_job_frame
ON depend (pk_job_depend_er, pk_frame_depend_er)
WHERE b_active = true
AND pk_frame_depend_er IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_depend_active_er_job_layer
ON depend (pk_job_depend_er, pk_layer_depend_er)
WHERE b_active = true
 AND pk_frame_depend_er IS NULL
 AND pk_layer_depend_er IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_depend_active_er_job_only
ON depend (pk_job_depend_er)
WHERE b_active = true
AND pk_frame_depend_er IS NULL
AND pk_layer_depend_er IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_frame_depend_candidates
