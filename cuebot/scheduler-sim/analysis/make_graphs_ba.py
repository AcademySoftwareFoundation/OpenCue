"""before(inline-dispose) vs after(batch-reconcile) -- 3 graphs: reads/s, writes/s, health."""
import os
import re
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
CMP = os.environ.get("SIM_BENCH_DIR", "/tmp/cmp2")
RUNS = [("before", "before: inline-dispose", "#7f7f7f"),
        ("after",  "after: batch-reconcile", "#1f77b4")]
WATCH = 185
def sod(h):
...
def orphan_series(tag):
    rx = re.compile(r"\[(\d\d:\d\d:\d\d)\].*orphan=\s*(\d+)"); pts = []
    try:
        for ln in open(f"{CMP}/{tag}_sim.log", errors="ignore"):
            m = rx.search(ln)
            if m: pts.append((sod(m.group(1)), int(m.group(2))))
    except FileNotFoundError: return []
    if not pts: return []
    t0 = pts[0][0]; return [(t-t0, v) for t, v in pts if 0 <= t-t0 <= WATCH]
def rows(tag):
...
line("Postgres read rate  (rows returned + fetched / s)", "ba_reads.png", "rows read / s", "reads")
line("Postgres write rate  (rows inserted + updated + deleted / s)", "ba_writes.png", "rows written / s", "writes")
...
plt.tight_layout(); plt.savefig(f"{CMP}/ba_health.png", dpi=110); plt.close(); print("wrote ba_health.png")
print("\n=== summary (mean / peak over window) ===")
def st(s): v=[x[1] for x in s]; return (sum(v)/len(v), max(v)) if v else (0,0)
for tag, label, _ in RUNS:
    u=st(util_series(tag)); orp=st(orphan_series(tag)); rd=st(R[tag]["reads"]); wr=st(R[tag]["writes"])
    rb=st(R[tag]["rollbacks"]); lw=st(R[tag]["lockwait"]); dl=st(R[tag]["deadlocks"]); ac=st(R[tag]["active"])
    print(f"{label}: util {u[0]:.0f}/{u[1]:.0f}% | orphan {orp[0]:.0f}/{orp[1]:.0f} | reads {rd[0]:.0f}/{rd[1]:.0f} | writes {wr[0]:.0f}/{wr[1]:.0f} "
          f"| rollbacks/s {rb[0]:.2f}/{rb[1]:.1f} | lock-wait {lw[0]:.2f}/{lw[1]:.0f} | deadlk/s {dl[0]:.3f} | active {ac[0]:.1f}/{ac[1]:.0f}")
