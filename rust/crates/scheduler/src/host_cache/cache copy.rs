...
impl HostCache {
    ...
    fn gen_memory_key(memory: u64) -> MemoryKey {
        memory / bytesize::kb(CONFIG.host_cache.memory_key_divisor.as_u64())
    }
}
...
mod tests {
    ...
    fn test_checkout_insufficient_cores() {
        let mut cache = HostCache::new();
        let host_id = Uuid::new_v4();
        let host = create_test_host(host_id, 2, 8192);
    ...
    fn test_checkout_insufficient_memory() {
        let mut cache = HostCache::new();
        let host_id = Uuid::new_v4();
        let host = create_test_host(host_id, 4, 4096);

        cache.insert(host);

        let result = cache.checkout(
            CoreSize(2),
            8192, // Request more memory than available
            |_| true,
        );
    ...
    fn test_checkout_validation_fails() {
        let mut cache = HostCache::new();
        let host_id = Uuid::new_v4();
        let host = create_test_host(host_id, 4, 8192);
    ...
    fn test_checkout_already_checked_out() {
        ...
        let result1 = cache.checkout(CoreSize(2), 4096, |_| true);
        assert!(result1.is_ok());

        // Second checkout should fail because host is already checked out
        let result2 = cache.checkout(CoreSize(2), 4096, |_| true);
    ...
    fn test_checkin() {
        ...
        // Checkout the host
        let result = cache.checkout(CoreSize(2), 4096, |_| true);
        assert!(result.is_ok());
        assert!(cache.checked_out_hosts.contains_key(&host_id));
    ...
    fn test_is_checked_out_true_when_recently_checked_out() {
        ...
        cache.insert(host);
        let _ = cache.checkout(CoreSize(2), 4096, |_| true);
    ...
    fn test_find_candidate_with_multiple_hosts() {
        ...
        let host1 = create_test_host(host1_id, 2, 4096);
        
        let host2_id = Uuid::new_v4();
        let host2 = create_test_host(host2_id, 4, 8192);
        
        let host3_id = Uuid::new_v4();
        let host3 = create_test_host(host3_id, 8, 16384);
        ...
        let result = cache.checkout(CoreSize(3), 6144, |_| true);
        assert!(result.is_ok());
        
        let chosen_host = result.unwrap();
        assert!(chosen_host.idle_cores.value() >= 3);
        assert!(chosen_host.idle_memory >= 6144);
    }

    #[test]
    fn test_gen_memory_key() {
        // With 2GB divisor, these small memory amounts will result in key 0
    ...
    fn test_multiple_hosts_same_resources() {
        ...
        let host1_id = Uuid::new_v4();
        let host1 = create_test_host(host1_id, 4, 8192);
