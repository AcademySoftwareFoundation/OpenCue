...
class CueGuiPlugin(ABC):
    ...
    def __init__(
        ...
        """
        Initialize the plugin with a job, optional parent QWidget, and optional config.

        Args:
            job (Optional[object]): The Cue job the plugin will interact with.
            parent (Optional[QWidget]): Parent widget for any UI elements.
    ...
    @abstractmethod
    def menuAction(self) -> Union[QAction, List[QAction]]:
        """
        Return a QAction or a list of QActions to be inserted into the CueGUI menu.

        Returns:
            Union[QAction, List[QAction]]: One or more actions to display in job/layer/frame menus.
        """
        pass
