...
class LayerMonitorTree(cuegui.AbstractTreeWidget.AbstractTreeWidget):
    ...
    def __init__(self, parent):
        ...
        self.addColumn(
            ...
            65,
            id=19,
            data=lambda layer: cuegui.Utils.secondsToHHMMSS(
                layer.data.layer_stats.avg_frame_sec
            ),
    ...
    def contextMenuEvent(self, e):
        ...
        readonly = (
            cuegui.Constants.FINISHED_JOBS_READONLY_LAYER
            and self.__job
            and self.__job.state() == job_pb2.FINISHED
        )
            ...
            try:
                if int(self.app.settings.value("DisableDeeding", 0)) == 0:
                    if len({layer.data.range for layer in __selectedObjects}) == 1:
                        reorder_action = self.__menuActions.layers().addAction(
                            menu, "reorder"
                        )
                        reorder_action.setEnabled(not readonly)
                    self.__menuActions.layers().addAction(menu, "stagger").setEnabled(
                        not readonly
                    )
        ...
        menu.addSeparator()
        self.__menuActions.layers().addAction(menu, "setProperties").setEnabled(
            not readonly
        )
        menu.addSeparator()
        ...
        if [layer for layer in __selectedObjects if layer.data.layer_stats.dead_frames]:
            menu.addSeparator()
            self.__menuActions.layers().addAction(menu, "retryDead").setEnabled(
                not readonly
            )
