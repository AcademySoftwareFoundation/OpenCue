...
class CueJobMonitorTree(cuegui.AbstractTreeWidget.AbstractTreeWidget):
    ...
    def __init__(self, parent):
        ...
        self.addColumn("Gpu", 55, id=6,
                       data=lambda job: "%.02f" % job.data.job_stats.reserved_gpu,
                       sort=lambda job: job.data.job_stats.reserved_gpu,
                       tip="The number of reserved gpu.")
...
#                       delegate=JobBookingBarDelegate)
        ...
        self.addColumn("Max", 38, id=11,
                       ...
                       tip="The maximum number of running cores that the cuebot\n"
                           "will allow.")
        self.addColumn("Min Gpu", 38, id=12,
                       data=lambda job: "%.0f" % job.data.min_gpu,
                       sort=lambda job: job.data.min_gpu,
                       tip="The minimum number of running gpu that the cuebot\n"
                           "will try to maintain.")
        self.addColumn("Max Gpu", 38, id=13,
                       data=lambda job: "%.0f" % job.data.max_gpu,
                       sort=lambda job: job.data.max_gpu,
                       tip="The maximum number of running gpu that the cuebot\n"
                           "will allow.")
        self.addColumn("Age", 50, id=14,
        ...
        self.addColumn("MaxRss", 60, id=17,
        ...
        self.addColumn("_Blank", 20, id=18,
                       tip="Spacer")
        self.addColumn("Progress", 0, id=19,
        ...
        for itemType in [cuegui.Constants.TYPE_GROUP, cuegui.Constants.TYPE_ROOTGROUP]:
            ...
            self.addColumn("", 0, id=6,
                           data=lambda group: "%.2f" % group.data.stats.reserved_gpu)
            self.addColumn("", 0, id=7,
                           data=lambda group: group.data.stats.waiting_frames)
            self.addColumn("", 0, id=8)
            self.addColumn("", 0, id=9)
            self.addColumn("", 0, id=10, data=lambda group: (group.data.min_cores or ""))
            self.addColumn("", 0, id=11, data=lambda group: (group.data.max_cores > 0 and group.data.max_cores or ""))
            self.addColumn("", 0, id=12, data=lambda group: (group.data.min_gpu or ""))
            self.addColumn("", 0, id=13, data=lambda group: (group.data.max_gpu > 0 and group.data.max_gpu or ""))
