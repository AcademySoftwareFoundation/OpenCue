...
import cuebot.proto.comment_pb2
...
class CommentsTests(unittest.TestCase):
    ...
    def setUp(self, getStubMock):
        ...
        commentProto = cuebot.proto.comment_pb2.Comment(
        ...
        getStubMock.return_value.GetComments.return_value = \
            cuebot.proto.job_pb2.JobGetCommentsResponse(
                comments=cuebot.proto.comment_pb2.CommentSeq(comments=[commentProto]))

        self.job_name = "fooJob"
        self.job = opencue.wrappers.job.Job(cuebot.proto.job_pb2.Job(name=self.job_name))
