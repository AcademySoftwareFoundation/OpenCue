...
class FilterDialogTests(unittest.TestCase):
    ...
    def setUp(self, getStubMock):
        ...
        self.show = opencue.wrappers.show.Show(cuebot.proto.show_pb2.Show(name='fooShow'))
        filterProto = cuebot.proto.filter_pb2.Filter(
        ...
        getStubMock.return_value.GetFilters.return_value = \
            cuebot.proto.show_pb2.ShowGetFiltersResponse(
                filters=cuebot.proto.filter_pb2.FilterSeq(filters=[filterProto]))
    ...
    def test_shouldAddFilter(self, getTextMock):
        ...
        self.show.createFilter = mock.Mock(
            return_value=opencue.wrappers.filter.Filter(
                cuebot.proto.filter_pb2.Filter(id=newFilterId, name=newFilterName)))
...
class FilterMonitorTreeTests(unittest.TestCase):
    ...
    def setUp(self, getStubMock):
        ...
        show = opencue.wrappers.show.Show(cuebot.proto.show_pb2.Show(name='fooShow'))
        filters = [
            cuebot.proto.filter_pb2.Filter(
                id='filter-one-id', name='filterOne', order=1, enabled=True),
            cuebot.proto.filter_pb2.Filter(
        ...
        getStubMock.return_value.GetFilters.return_value = \
            cuebot.proto.show_pb2.ShowGetFiltersResponse(
                filters=cuebot.proto.filter_pb2.FilterSeq(filters=filters))
...
class MatcherMonitorTreeTests(unittest.TestCase):
    ...
    def setUp(self, getStubMock):
        ...
        self.matchers = [
            cuebot.proto.filter_pb2.Matcher(
                id='matcher-one-id',
                subject=cuebot.proto.filter_pb2.SHOW,
                type=cuebot.proto.filter_pb2.IS,
                input='showName'),
            cuebot.proto.filter_pb2.Matcher(
                id='matcher-two-id',
                subject=cuebot.proto.filter_pb2.JOB_NAME,
                type=cuebot.proto.filter_pb2.CONTAINS,
                input='jobNameSnippet'),
        ]
        self.matcherWrappers = [
            opencue.wrappers.filter.Matcher(matcher) for matcher in self.matchers]
        self.filter = opencue.wrappers.filter.Filter(cuebot.proto.filter_pb2.Filter())
    ...
    def test_shouldAddMatcher(self, getItemMock, getTextMock):
        matcherSubject = cuebot.proto.filter_pb2.FACILITY
        matcherType = cuebot.proto.filter_pb2.CONTAINS
    ...
    def test_shouldAddMultipleMatchers(self, getItemMock, textEditDialogMock):
        matcherSubject = cuebot.proto.filter_pb2.SHOT
        matcherType = cuebot.proto.filter_pb2.IS
        matcherText = 'SHOt01 \n\nshot02\nShot03'
        self.filter.getMatchers = mock.Mock(return_value=self.matcherWrappers)
        self.filter.createMatcher = mock.Mock(
            return_value=opencue.wrappers.filter.Matcher(
