...
// Set a job's minimum GPUs. Request: { job, val }.
// RPC: /job.JobInterface/SetMinGpus.
export async function POST(request: NextRequest) {
  const endpoint = "/job.JobInterface/SetMinGpus";
  ...
  if (!jsonBody?.job || typeof jsonBody.val !== 'number') {
    return NextResponse.json({ error: 'Invalid request body: job and numeric val required' }, { status: 400 });
  }
