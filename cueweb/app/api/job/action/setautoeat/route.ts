...
export async function POST(request: NextRequest) {
  const endpoint = "/job.JobInterface/SetAutoEat";
  ...
  if (!jsonBody || typeof jsonBody !== 'object' || !jsonBody.job || typeof jsonBody.value !== 'boolean') {
    return NextResponse.json({ error: 'Invalid request body (need {job, value:boolean})' }, { status: 400 });
  }
