...
// List a show's dispatcher filters (View Filters dialog). Request: { show }.
// RPC: /show.ShowInterface/GetFilters. The gateway nests the seq under
// filters.filters.
export async function POST(request: NextRequest) {
  const endpoint = "/show.ShowInterface/GetFilters";
  ...
  return NextResponse.json({ data: responseData.data?.filters?.filters ?? [] }, { status: response.status });
}
