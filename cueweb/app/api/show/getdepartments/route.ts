...
// List a show's departments (Task Properties dialog). Request: { show }.
// RPC: /show.ShowInterface/GetDepartments (nested under departments.departments).
export async function POST(request: NextRequest) {
  const endpoint = "/show.ShowInterface/GetDepartments";
  ...
  return NextResponse.json({ data: responseData.data?.departments?.departments ?? [] }, { status: response.status });
}
