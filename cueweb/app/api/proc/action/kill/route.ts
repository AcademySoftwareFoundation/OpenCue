...
// Kill a single proc (Proc monitor "Kill"). Request: { proc }.
// RPC: /host.ProcInterface/Kill.
export async function POST(request: NextRequest) {
  const endpoint = "/host.ProcInterface/Kill";
  ...
  if (!jsonBody?.proc) {
    return NextResponse.json({ error: 'Invalid request body: proc required' }, { status: 400 });
  }
