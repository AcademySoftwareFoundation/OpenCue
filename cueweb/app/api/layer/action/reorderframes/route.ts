...
// Reorder a layer's frames (CueGUI LayerActions.reorder). order is an Order
// enum name: "FIRST" | "LAST" | "REVERSE".
// RPC: /job.LayerInterface/ReorderFrames. Request: { layer, range, order }.
export async function POST(request: NextRequest) {
  const endpoint = "/job.LayerInterface/ReorderFrames";
  ...
  if (!jsonBody?.layer || typeof jsonBody.range !== 'string' || typeof jsonBody.order !== 'string') {
    return NextResponse.json({ error: 'Invalid request body: layer, range, order required' }, { status: 400 });
  }
