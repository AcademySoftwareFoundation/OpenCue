...
// Mirrors Layer.createDependencyOnFrame (pycue) ->
...
export async function POST(request: NextRequest) {
  ...
  if (!jsonBody || typeof jsonBody !== 'object' || Array.isArray(jsonBody) || !jsonBody.layer || !jsonBody.frame) {
    return NextResponse.json({ error: 'Invalid request body (need {layer, frame})' }, { status: 400 });
  }
