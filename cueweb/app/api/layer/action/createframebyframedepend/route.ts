...
// Mirrors Layer.createFrameByFrameDependency (pycue) ->
// LayerInterface.CreateFrameByFrameDependency. Used by the Frame By
...
export async function POST(request: NextRequest) {
  ...
  if (!jsonBody || typeof jsonBody !== 'object' || Array.isArray(jsonBody) || !jsonBody.layer || !jsonBody.depend_layer) {
    return NextResponse.json({ error: 'Invalid request body (need {layer, depend_layer, any_frame?})' }, { status: 400 });
  }
