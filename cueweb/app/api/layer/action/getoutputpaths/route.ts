...
// Return a layer's registered output paths (CueGUI Preview reads rendered
// outputs from these). A browser can't render the preview images, so CueWeb
// surfaces the paths so the user can locate them.
// RPC: /job.LayerInterface/GetOutputPaths. Request: { layer }.
export async function POST(request: NextRequest) {
  const endpoint = "/job.LayerInterface/GetOutputPaths";
