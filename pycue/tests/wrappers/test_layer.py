...
import mock
from opencue_proto import depend_pb2, job_pb2
...
class LayerTests(unittest.TestCase):
    ...
    def testEnableMemoryOptimizerTrue(self, getStubMock):
        stubMock = mock.Mock()
        stubMock.EnableMemoryOptimizer.return_value = (
            job_pb2.LayerEnableMemoryOptimizerResponse()
        )
        getStubMock.return_value = stubMock
    ...
    def testGetFrames(self, getStubMock):
        ...
        stubMock.GetFrames.assert_called_with(
            job_pb2.LayerGetFramesRequest(
                layer=layer.data, s=opencue.search.FrameSearch.criteriaFromOptions()
            ),
            timeout=mock.ANY,
        )
    ...
    def testGetWhatDependsOnThis(self, getStubMock):
        ...
        stubMock = mock.Mock()
        stubMock.GetWhatDependsOnThis.return_value = (
            job_pb2.LayerGetWhatDependsOnThisResponse(
        ...
        stubMock.GetWhatDependsOnThis.assert_called_with(
            job_pb2.LayerGetWhatDependsOnThisRequest(layer=layer.data), timeout=mock.ANY
        )
    ...
    def testGetWhatThisDependsOn(self, getStubMock):
        dependId = "dddd-ddd-dddd"
        stubMock = mock.Mock()
        stubMock.GetWhatThisDependsOn.return_value = (
            job_pb2.LayerGetWhatThisDependsOnResponse(
                depends=depend_pb2.DependSeq(depends=[depend_pb2.Depend(id=dependId)])
            )
        )
        getStubMock.return_value = stubMock
        ...
        stubMock.GetWhatThisDependsOn.assert_called_with(
            job_pb2.LayerGetWhatThisDependsOnRequest(layer=layer.data), timeout=mock.ANY
        )
        self.assertEqual(len(depends), 1)
        self.assertEqual(depends[0].id(), dependId)
    ...
    def testCreateDependencyOnJob(self, getStubMock):
        ...
        stubMock.CreateDependencyOnJob.return_value = (
            job_pb2.LayerCreateDependOnJobResponse(
        ...
        layer = opencue.wrappers.layer.Layer(job_pb2.Layer(name=TEST_LAYER_NAME))
        job = opencue.wrappers.job.Job(job_pb2.Job(id=jobId))
    ...
    def testCreateDependencyOnLayer(self, getStubMock):
        ...
        stubMock.CreateDependencyOnLayer.return_value = (
            job_pb2.LayerCreateDependOnLayerResponse(
                depend=depend_pb2.Depend(id=dependId)
            )
        )
        getStubMock.return_value = stubMock
        ...
        layer = opencue.wrappers.layer.Layer(job_pb2.Layer(name=TEST_LAYER_NAME))
        dependLayer = opencue.wrappers.layer.Layer(job_pb2.Layer(id=layerId))
        ...
        stubMock.CreateDependencyOnLayer.assert_called_with(
            job_pb2.LayerCreateDependOnLayerRequest(
                layer=layer.data, depend_on_layer=dependLayer.data
            ),
            timeout=mock.ANY,
        )
        self.assertEqual(depend.id(), dependId)
    ...
    def testCreateDependencyOnFrame(self, getStubMock):
        ...
        stubMock.CreateDependencyOnFrame.return_value = (
            job_pb2.LayerCreateDependOnFrameResponse(
                depend=depend_pb2.Depend(id=dependId)
            )
        )
        getStubMock.return_value = stubMock
        ...
        layer = opencue.wrappers.layer.Layer(job_pb2.Layer(name=TEST_LAYER_NAME))
        frame = opencue.wrappers.frame.Frame(job_pb2.Frame(id=frameId))
    ...
    def testCreateFrameByFrameDependency(self, getStubMock):
        ...
        stubMock.CreateFrameByFrameDependency.assert_called_with(
            job_pb2.LayerCreateFrameByFrameDependRequest(
                layer=layer.data, depend_layer=dependLayer.data, any_frame=False
            ),
            timeout=mock.ANY,
        )
        self.assertEqual(depend.id(), dependId)
    ...
    def testRegisterOutputPath(self, getStubMock):
        stubMock = mock.Mock()
        stubMock.RegisterOutputPath.return_value = (
            job_pb2.LayerRegisterOutputPathResponse()
        )
        getStubMock.return_value = stubMock
    ...
    def testReorderFrames(self, getStubMock):
        ...
        stubMock.ReorderFrames.assert_called_with(
            job_pb2.LayerReorderFramesRequest(
                layer=layer.data, range=frameRange, order=order
            ),
            timeout=mock.ANY,
        )
    ...
    def testStaggerFrames(self, getStubMock):
        ...
        stubMock.StaggerFrames.assert_called_with(
            job_pb2.LayerStaggerFramesRequest(
                layer=layer.data, range=frameRange, stagger=stagger
            ),
            timeout=mock.ANY,
        )
