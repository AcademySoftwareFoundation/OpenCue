...
class AllocTests(unittest.TestCase):
    ...
    def testCreateAlloc(
        self, getFacilityMock, createAllocMock, getStubMock, findAllocMock
    ):
        tagName = "random-tag"
        args = self.parser.parse_args(
    ...
    def testRenameAlloc(self, getStubMock, findAllocMock):
        ...
        args = self.parser.parse_args(
            ["-rename-alloc", oldFullName, newFullName, "-force"]
        )
        allocMock = mock.Mock()
        findAllocMock.return_value = allocMock
    ...
    def testInvalidRenameAlloc(self, getStubMock, findAllocMock):
        ...
        args = self.parser.parse_args(
            ["-rename-alloc", oldFullName, invalidNewName, "-force"]
        )
    ...
    def testReparentHosts(self, getStubMock, findAllocMock):
        ...
        args = self.parser.parse_args(
            ["-transfer", srcAllocName, dstAllocName, "-force"]
        )
        ...
        findAllocMock.assert_has_calls(
            [mock.call(srcAllocName), mock.call(dstAllocName)]
        )
...
class HostTests(unittest.TestCase):
    ...
    def testMoveHost(self, findAllocMock, getStubMock, hostSearchMock):
        allocName = "%s.%s" % (TEST_FACILITY, TEST_ALLOC)
        args = self.parser.parse_args(
            ["-move", allocName, "-host", TEST_HOST, "-force"]
        )
...
class SubscriptionTests(unittest.TestCase):
    ...
    def testSetSize(self, getStubMock, findSubMock):
        ...
        newSize = 200
        args = self.parser.parse_args(
            ["-size", TEST_SHOW, allocName, str(newSize), "-force"]
        )
        subName = "%s.%s" % (allocName, TEST_SHOW)
        subMock = mock.Mock()
        findSubMock.return_value = subMock
    ...
    def testSetBurst(self, getStubMock, findSubMock):
        ...
        newBurstSize = 847
        args = self.parser.parse_args(
            ["-burst", TEST_SHOW, allocName, str(newBurstSize), "-force"]
        )
        subName = "%s.%s" % (allocName, TEST_SHOW)
        subMock = mock.Mock()
        findSubMock.return_value = subMock
    ...
    def testSetBurstPercentage(self, getStubMock, findSubMock):
        ...
        newBurstPerc = "20%"
        args = self.parser.parse_args(
            ["-burst", TEST_SHOW, allocName, newBurstPerc, "-force"]
        )
        subName = "%s.%s" % (allocName, TEST_SHOW)
        subMock = mock.Mock()
        ...
        findSubMock.assert_called_with(subName)
        subMock.setBurst.assert_called_with(
            originalSize * (1 + float(newBurstPerc[:-1]) / 100)
        )
...
class ProcTests(unittest.TestCase):
    ...
    def testListProcs(self, getStubMock, procSearchMock):
        ...
        procSearchMock.byOptions.assert_called_with(
            alloc=[TEST_ALLOC],
            duration=[
                opencue.api.criterion_pb2.GreaterThanIntegerSearchCriterion(value=5400)
            ],
            host=[TEST_HOST],
            job=[TEST_JOB],
            limit=resultsLimit,
