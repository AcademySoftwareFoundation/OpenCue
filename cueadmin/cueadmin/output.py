...
def displayServices(services):
    ...
    print(
        service_format % ("Name", "Can Thread", "Min Cores Units", "Min Memory", "Tags")
    )
    for srv in services:
        print(
            service_format
...
def displayJobs(jobs):
    ...
    for job in jobs:
        ...
        print(
            job_format
            % (
                cueadmin.format.cutoff(name, 52),
...
def displayJobInfo(job):
    ...
    print(
        "%13s: %0.2f / %0.2f"
        % ("Min/Max cores", job.data.min_cores, job.data.max_cores)
    )
    print("")
...
def displayFrames(frames):
    ...
    for frame in frames:
        ...
        if frame.data.start_time:
            duration = cueadmin.format.formatDuration(
                cueadmin.format.findDuration(
                    frame.data.start_time, frame.data.stop_time
                )
            )
        else:
            duration = ""
    ...
    if len(frames) == 1000:
        print(
            "Warning: Only showing first 1000 matches. See frame query options to "
            "limit your results."
        )
