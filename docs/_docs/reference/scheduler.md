...
**Time to First Frame**:
- Traditional Cuebot: 5-30 seconds (depends on host report interval)
- Scheduler: 1-10 seconds (proactive loop, depends on `monitor_interval`)

**Dispatch Throughput**:
- Limited by database transaction rate
- Typical: 10-50 dispatches/second per instance
- Peak: 100+ dispatches/second with tuned configuration
...
│   ├── cluster.rs                # Cluster feed and iteration
│   ├── cluster_key.rs            # Cluster key types and tags
│   ├── config/                   # Configuration management
│   │   ├── mod.rs                # Config loading and parsing
│   │   └── error.rs              # Config error types
│   ├── dao/                      # Database access layer
│   │   ├── mod.rs                # DAO module root
│   │   ├── job_dao.rs            # Job queries
│   │   ├── layer_dao.rs          # Layer queries
│   │   ├── frame_dao.rs          # Frame locking and updates
│   │   ├── host_dao.rs           # Host queries
│   │   ├── proc_dao.rs           # Proc creation
│   │   ├── allocation_dao.rs     # Allocation queries
│   │   └── helpers.rs            # Utility functions
│   ├── host_cache/               # Host caching system
│   │   ├── mod.rs                # Cache module root
│   │   ├── cache.rs              # B-tree cache implementation
│   │   ├── store.rs              # Global host store (DashMap)
│   │   ├── service.rs            # Actix actor service
│   │   └── messages.rs           # Actor messages (CheckOut, CheckIn)
│   ├── pipeline/                 # Scheduling pipeline
│   │   ├── mod.rs                # Pipeline module root
│   │   ├── entrypoint.rs         # Main scheduling loop
│   │   ├── matcher.rs            # Matching service (layer to host)
│   │   ├── layer_permit.rs       # Layer permit system
│   │   └── dispatcher/           # Frame dispatcher
│   │       ├── mod.rs            # Dispatcher module root
│   │       ├── actor.rs          # Actix dispatcher actor
│   │       ├── frame_set.rs      # Frame grouping for dispatch
│   │       ├── messages.rs       # Dispatcher messages
│   │       └── error.rs          # Dispatch error types
│   ├── metrics/                  # Prometheus metrics
│   │   └── mod.rs                # Metrics definitions and collection
│   ├── allocation/               # Allocation service
│   │   └── mod.rs                # Subscription management
│   ├── models/                   # Data models
│   │   └── mod.rs                # Host, Layer, Frame, etc.
│   └── pgpool.rs                 # PostgreSQL connection pool
├── resources/
│   └── schema.sql                # Database schema (reference)
├── config/
│   └── scheduler.yaml            # Sample configuration
