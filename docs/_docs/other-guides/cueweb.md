...
7. **Secure user authentication + Role-Based Access Control:**
   - Authentication through built-in **local** username/password, **Okta**, **Google**, **GitHub**, and **LDAP**. Choose any subset via `NEXT_PUBLIC_AUTH_PROVIDER` (comma-separated). Empty = sandbox mode (no login). Other providers can be added via [NextAuth.js](https://next-auth.js.org/) configuration.
