...
def _verify_check(name, gdir, logp, cblog):
    ...
    if name == "COMPLETIONSTORM":
        ...
        # post-op queue, and the tick must stay calm with zero dropped
        # completions.
        try:
            txt = open(logp, errors="ignore").read()
        except Exception:
...
def run_verify():
    ...
    scenarios = [
        ...
        # ~145/s; the queue is pinned small (300) through the property env so
        # the overflow is reached in seconds. Fail-first: the bounded queue's
        # caller-runs overflow files on Maestro and the tick trebles.
        # Fixed: the door refuses before acking, RQD redials, the tick stays
        # calm and nothing is dropped.
        ("COMPLETIONSTORM", ["--hosts", "0,0,80", "--cuebots", "1",
         ...
         {"SIM_DUR_LONG_S": "1", "SIM_STORM_JOBS": "400",
          ...
          "MAESTRO_POST_COMPLETE_QUEUE_SIZE": "300",
...
def main():
    ...
    watch = (args.strand or args.priority_starve or args.priority_spread
             ...
             or args.gpustrand_test or args.showtier_test or args.slice_test
