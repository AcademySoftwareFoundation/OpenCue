...
| **FORWARD** | the completion-forward relay on the isolated-leader rollout topology: the managed cuebot is off the report spread (no RQD knows its address) and both legacy cuebots forward the managed show's completions to it over the report gRPC (`maestro.forward_completions_to`); forwarding must carry the show into the isolated drain (fail-first: flag off = zero and FAIL), steady fallbacks stay near zero, the breaker falls back and forwarding resumes around a mid-run SIGKILL of the managed cuebot, the ambiguity arm's tiny-deadline double-processing resolves effectively-once, and neither dispatcher books the other's shows |
