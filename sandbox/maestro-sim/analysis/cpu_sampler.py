...
def snap():
    ...
    g={"cuebot":0,"postgres":0,"python":0}
...
open(OUT,"w").write("ts,total_cpu_pct,cuebot_cores,postgres_cores,python_cores\n")
while True:
    ...
    open(OUT,"a").write(f"{time.strftime('%H:%M:%S')},{cpu:.1f},{c('cuebot'):.2f},{c('postgres'):.2f},{c('python'):.2f}\n")
